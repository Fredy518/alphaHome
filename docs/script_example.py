...
使用 TaskUpdaterBase 基类实现的股票月线数据更新工具。
支持以下功能：
1. 按月份更新数据
2. 支持指定日期范围更新
3. 支持多种更新模式（增量/全量）
...
# 定义目标任务名称常量
TARGET_TASK_NAME = "tushare_stock_monthly"

class MonthlyUpdater(TaskUpdaterBase):
    """
    股票月线数据更新工具
    
    继承自 TaskUpdaterBase，用于更新股票月线数据。
    支持按月份更新、指定日期范围更新、全量更新等多种更新模式。
    """
...
async def main():
    ...
    parser = argparse.ArgumentParser(description="更新股票月线数据")
...
if __name__ == "__main__":
    ...
    #
    # 2. 指定日期范围更新
    #    python script_example.py --start-date 20230101 --end-date 20230630
    #
    # 3. 全量更新（从最早数据到当前）
    #    python script_example.py --full-update
    asyncio.run(main()) 