...
│   │   │   └── balance_mv.py      # 资产负债表物化视图
│   │   └── market/                # 市场数据物化视图
│   │       ├── technical_mv.py    # 技术指标聚合视图
│   │       └── sector_mv.py       # 行业板块聚合视图
│   ├── operations/                # 物化视图操作层
│   │   ├── refresh_ops.py         # 刷新操作
│   │   ├── validation_ops.py      # 验证操作
│   │   └── incremental_ops.py     # 增量操作
│   ├── engine/                    # 物化视图引擎层
│   │   └── mv_engine.py           # 物化视图专用引擎
│   └── refresh/                   # 刷新管理子系统
│       ├── scheduler.py           # 智能刷新调度器
│       ├── monitor.py             # 性能监控组件
│       └── validator.py           # 刷新结果验证器
├── domain/                        # 现有域服务层
├── tasks/                         # 现有processor任务层
├── operations/                    # 现有操作层
└── engine/                        # 现有引擎层
```
