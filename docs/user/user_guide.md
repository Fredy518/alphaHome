...
| 标签页 | 用途 |
| --- | --- |
| 日常更新 | 预览并一键按依赖顺序执行智能增量；包含 ETF 候选池月度 AI 维护 |
...
“一键智能增量更新”以 `rawdata.others_calendar` 的 SSE 日历判断交易日。日常任务每天纳入，周/月/季度及全量型任务只在周末或其他非交易日纳入；被 GUI 明确隐藏的分钟线、历史兼容等手工任务始终不会被一键运行，并会在计划详情中列明。更新按声明的领域依赖执行；上游发生真实失败时，依赖它的下游会显示“前置更新域未成功”并停止启动，避免消费旧数据。

ETF 候选池是 Features 之后的月度更新域：每月 5 日起检查，当月已有一次成功结果便自动跳过。到期时它重新验证产品事实截止日和候选覆盖，按需调用 DeepSeek，完成本地 JSON/枚举/置信度门禁后原子写入版本化快照；人工确认与人工拒绝不会被自动覆盖。该域只维护研究候选，没有资金或下单权限。FundPos 与它同属 Features 的下游分支，固定使用影子模式；候选池模型失败不会阻止独立的 FundPos 估算。
