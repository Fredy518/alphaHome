...
class TushareIndexSwDailyTask(TushareTask):
    ...
    description = "获取申万行业日线行情数据"
    table_name = "tushare_index_swdaily"
    primary_keys = ["ts_code", "trade_date"]
    date_column = "trade_date"
    default_start_date = "20050101" # 申万行业指数通常在此日期后开始
    ...
    async def get_batch_list(self, **kwargs: Any) -> List[Dict]:
        ...
        self.logger.info(f"任务 {self.name}: 生成批处理列表，范围: {start_date} 到 {end_date}, 指数代码: {ts_code if ts_code else '所有'}")
    ...
    async def validate_data(self, df: pd.DataFrame, **kwargs: Any) -> pd.DataFrame:
        ...
        if missing_cols:
            ...
            # 可以选择抛出异常或返回空 DataFrame
            raise ValueError(error_msg) 
            # return pd.DataFrame() # 或者返回空

        # 可选：检查价格/成交量/成交额是否为负数
        numeric_cols_to_check = ['open', 'low', 'high', 'close', 'volume', 'amount']
        for col in numeric_cols_to_check:
            if col in df.columns:
                negative_count = (df[col].dropna() < 0).sum()
                if negative_count > 0:
                    self.logger.warning(f"任务 {self.name}: 数据验证发现 {negative_count} 条记录的 {col} 为负值。")
                    # 可以选择过滤或仅记录
                    # df = df[df[col] >= 0]

        self.logger.info(f"任务 {self.name}: 数据验证通过 (或仅记录警告)，保留 {len(df)} 条记录。")
        return df 