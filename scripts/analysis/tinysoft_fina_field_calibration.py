...
class FieldStats:
    ...
    def numeric_ratio(self) -> float:
        if self.non_empty_rows <= 0:
            return 0.0
        return self.numeric_rows / self.non_empty_rows
...
async def _probe(
    ...
    for ts_code in symbols:
        ...
        stats.numeric_rows += int(non_empty.map(_is_numeric).sum())
        for v in non_empty.astype(str).str.strip().head(20):
...
def _build_report(
    *,
    ...
    lines: List[str] = []
    lines.append(f"# TinySoft 财务字段口径校准报告（{datetime.now().strftime('%Y-%m-%d %H:%M:%S')}）")
    lines.append("")
    lines.append("## 1. 运行参数")
    ...
    lines.append("")
    lines.append("| 表达式 | 总行数 | 非空行数 | 非空率 | 数值行数 | 数值率(相对非空) | 非空股票覆盖率 | 查询错误数 |")
    lines.append("|---|---:|---:|---:|---:|---:|---:|---:|")
    for s in stats_list:
        lines.append(
            "| {expr} | {total} | {non_empty} | {ratio:.4f} | {numeric_rows} | {numeric_ratio:.4f} | {cov:.4f} | {errors} |".format(
                expr=s.expression,
                total=s.total_rows,
                non_empty=s.non_empty_rows,
                ratio=s.non_empty_ratio,
                numeric_rows=s.numeric_rows,
                numeric_ratio=s.numeric_ratio,
    ...
    if recommended is None:
        ...
        lines.append(
            "- 推荐依据：非空股票覆盖率=`{:.4f}`，非空行数=`{}`，数值行数=`{}`。".format(
                recommended.symbol_coverage,
                recommended.non_empty_rows,
                recommended.numeric_rows,
            )
        )
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="TinySoft 财务字段口径校准脚本")
    ...
    parser.add_argument(
        "--output",
        default=f"docs/tasks/tinysoft_fina_field_calibration_{datetime.now().strftime('%Y%m%d')}.md",
    )
