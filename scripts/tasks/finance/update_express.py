...
使用 TaskUpdaterBase 基类实现的业绩快报数据更新工具。
支持以下功能：
1. 按季度或年度更新数据
2. 支持指定报告期更新
3. 支持多种更新模式（增量/全量）
...
# 定义目标任务名称常量
TARGET_TASK_NAME = "tushare_fina_express"

class ExpressUpdater(TaskUpdaterBase):
    """业绩快报数据更新器"""
    ...
    def __init__(self):
        super().__init__(
            ...
            task_type="业绩快报",
            description="业绩快报数据更新工具",
            support_report_period=True
        )
        self.logger = logging.getLogger('update_express')
...
async def main():
    ...
    updater = ExpressUpdater()
    start_time = datetime.now()
    updater.logger.info(f"开始执行业绩快报 ({updater.task_name}) 数据更新...")
    ...
    try:
        ...
        updater.logger.error(f"业绩快报数据更新过程中发生错误: {str(e)}", exc_info=True)
        if result is None:
    ...
    updater.logger.info(f"业绩快报数据更新执行完毕。总耗时: {duration}")

if __name__ == "__main__":
    """
    使用示例:
    1. 默认增量更新（智能模式，回溯1年）:
       python scripts/update_express_task.py
    2. 更新最近N个季度:
       python scripts/update_express_task.py --quarters 4
    3. 更新最近N年:
       python scripts/update_express_task.py --years 2
    4. 更新特定报告期:
       python scripts/update_express_task.py --report-period 202103
    5. 指定日期范围更新:
       python scripts/update_express_task.py --start-date 20230101 --end-date 20230630
    6. 全量更新:
       python scripts/update_express_task.py --full-update
    """
    asyncio.run(main()) 