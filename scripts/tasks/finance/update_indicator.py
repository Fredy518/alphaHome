...
使用 TaskUpdaterBase 基类实现的财务指标数据更新工具。
支持以下功能：
1. 按季度或年度更新数据
2. 支持指定报告期更新
3. 支持多种更新模式（增量/全量）
...
# 定义目标任务
TARGET_TASK_NAME = "tushare_fina_indicator" # 财务指标

class FinaIndicatorUpdater(TaskUpdaterBase):
    def __init__(self):
        super().__init__(
            task_name="tushare_fina_indicator",
            task_type="财务指标",
            description="财务指标数据更新工具",
...
if __name__ == "__main__":
    ...
    使用示例:
    1. 默认增量更新（智能模式，回溯1年）:
       python scripts/update_fina_indicator_task.py
    2. 更新最近N个季度:
       python scripts/update_fina_indicator_task.py --quarters 4
    3. 更新最近N年:
       python scripts/update_fina_indicator_task.py --years 2
    4. 更新特定报告期:
       python scripts/update_fina_indicator_task.py --report-period 202103
    5. 指定日期范围更新:
       python scripts/update_fina_indicator_task.py --start-date 20230101 --end-date 20230630
    6. 全量更新:
       python scripts/update_fina_indicator_task.py --full-update
    """
    asyncio.run(main())