...
log_filename = f"update_stockbasic_{datetime.now().strftime('%Y%m%d')}.log"
...
# from data_module.tasks.stock.tushare_stock_basic import TushareStockBasicTask # TaskFactory 会处理加载

# 定义目标任务名称常量
TARGET_TASK_NAME = "tushare_stock_basic"

class StockBasicUpdater(TaskUpdaterBase):
    """
    股票基本信息数据更新工具 (全量替换)
    """
    ...
    async def update_task(self, args: argparse.Namespace) -> Dict[str, Any]:
        ...
        try:
            ...
            # 注意：TaskFactory 在 get_task 时应已注入 DBManager
    ...
    def summarize_result(self, result: Dict[str, Any], args: argparse.Namespace):
        ...
        elif status == 'partial_success': # 处理 TushareTask 可能返回的状态
        ...
        elif status == 'failure': # 处理 TushareTask 可能返回的状态
            self.logger.error(f"状态: 失败")
            self.logger.info(f"更新行数: {rows}") # 即使失败也可能处理了部分行
            if 'failed_batches' in result:
        ...
        elif status == 'error': # 处理 updater.update_task 内部的错误
...
async def main():
    ...
    try:
        ...
        # 初始化 TaskFactory (传入 DBManager 实例)
        # TaskFactory 应负责将 db_manager 注入到创建的 Task 实例中
        await TaskFactory.initialize() # 移除 db_manager 和 api_token 参数
        ...
        # 关闭 TaskFactory 和数据库连接
        await TaskFactory.shutdown()
        if db_manager:
            await db_manager.close() # 使用 close 而不是 disconnect
