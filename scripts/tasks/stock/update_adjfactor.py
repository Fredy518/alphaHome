...
async def main():
    # ---- 命令行参数解析 ----
    parser = argparse.ArgumentParser(description='Tushare 股票复权因子数据更新工具 (支持全量和增量)')
    ...
    logger.info(f"开始执行 tushare_stock_adjfactor 数据更新...")
    ...
    task = None
    try:
        task_name = "tushare_stock_adjfactor"
        ...
        if args.full_update:
            ...
            if latest_date:
                ...
                # 如果没有数据，执行首次全量更新
                start_date_str = '19901219'  # Tushare 最早日期
    ...
    logger.info(f"tushare_stock_adjfactor 数据更新执行完毕。总耗时: {duration}")
