...
使用方法：
python scripts/production/factor_calculators/p_factor/calculate_p_factor_for_specific_dates.py --dates 2024-12-13 2024-12-20
"""
...
class SpecificDatePFactorCalculator:
    ...
    def _setup_logger(self):
        """设置日志"""
        import logging
        logger = logging.getLogger('SpecificDatePFactorCalculator')
    ...
    def calculate_p_factors_for_dates(self, dates: List[str]) -> dict:
        """为指定日期计算P因子
        ...
        Returns:
            计算结果统计
        """
        self.logger.info(f"开始为指定日期计算P因子: {dates}")
        ...
        for i, calc_date in enumerate(valid_dates, 1):
            ...
            try:
                # 获取该日期的交易股票列表
                stock_codes = self._get_trading_stocks_for_date(calc_date)

                if not stock_codes:
                    self.logger.warning(f"{calc_date}: 未找到交易股票，跳过")
                    results_by_date[calc_date] = {
                        'success_count': 0,
                        'failed_count': 0,
                        'stock_count': 0,
                        'error': 'no_trading_stocks'
                    }
                    continue

                self.logger.info(f"{calc_date}: 找到 {len(stock_codes)} 只交易股票")

                # 计算P因子
                result = self.calculator.calculate_p_factors_pit(calc_date, stock_codes)
    ...
    def _get_trading_stocks_for_date(self, calc_date: str) -> List[str]:
        """获取指定日期的交易股票列表
        ...
        """
        try:
            # 使用与P因子计算器相同的方法获取交易股票
            return self.calculator._get_trading_stock_codes(calc_date)
        except Exception as e:
            self.logger.error(f"获取 {calc_date} 交易股票失败: {e}")
            return []


def main():
    parser = argparse.ArgumentParser(description='P因子指定日期计算器')
    ...
    calculator = SpecificDatePFactorCalculator()
    result = calculator.calculate_p_factors_for_dates(args.dates)
