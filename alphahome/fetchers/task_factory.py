...
from ..common.config_manager import reload_config as _reload_config
...
class TaskFactory:
    ...
    async def initialize(cls, db_url=None):
        ...
        if not db_url:
            logger.error(
                "无法获取有效的数据库连接 URL (配置文件或环境变量均未设置)，TaskFactory 初始化失败。"
            )
    ...
    async def reload_config(cls):
        ...
        try:
            ...
            logger.exception(
                "重新加载 TaskFactory 配置时发生错误。TaskFactory 可能处于不稳定状态。"
            )
