...
class Task(ABC):
    ...
    async def execute(
        ...
        try:
            ...
            )  # <<< Pass stop_event (for potential overrides) >>>
    ...
    async def post_execute(
        self, result, stop_event: Optional[asyncio.Event] = None, **kwargs
    ):  # <<< Add stop_event >>>
        """任务执行后的清理工作"""
    ...
    def process_data(
        self, data, stop_event: Optional[asyncio.Event] = None, **kwargs
    ):  # <<< Add stop_event >>>
        """处理原始数据"""
    ...
    def validate_data(self, data):
        ...
        if not validation_passed:
            ...
            if nan_columns:
                self.logger.info(
                    f"以下列包含NaN值，可能影响验证结果: {', '.join(nan_columns)}"
                )
    ...
    async def save_data(
        ...
        if nan_count > 0:
            self.logger.warning(
                f"数据中包含 {nan_count} 个NaN值，这些值在保存到数据库时将被转换为NULL"
            )
        ...
        # 将数据保存到数据库
        affected_rows = await self._save_to_database(
            data, stop_event=stop_event
        )  # <<< Pass stop_event >>>
        if stop_event and stop_event.is_set():
    ...
    async def _create_table(
        ...
        for col_name, col_def in self.schema.items():
            if isinstance(col_def, dict):
                ...
                constraints_val = col_def.get(
                    "constraints"
                )  # 获取原始约束值 (可能为 None)
                # 仅当约束值不是 None 时，才将其转换为空格分隔的字符串，否则使用空字符串
                constraints_str = (
                    str(constraints_val).strip() if constraints_val is not None else ""
                )
                columns.append(
            ...
            for i, index_def in enumerate(self.indexes):
                if stop_event and stop_event.is_set():
                    raise asyncio.CancelledError(
                        f"任务在创建第 {i+1} 个自定义索引前被取消"
                    )
                if isinstance(index_def, dict):
                    index_name = index_def.get(
                        "name",
                        f"idx_{self.table_name}_{index_def['columns'].replace(',', '_')}",
                    )
        ...
        **kwargs,
        ...
        if end_date is None:
            ...
            if current_time.hour >= 18:
                ...
                self.logger.info(
                    f"未指定结束日期，当前时间晚于18:00，使用当前日期: {end_date}"
                )
            else:
                ...
                self.logger.info(
                    f"未指定结束日期，当前时间早于18:00，使用昨天日期: {end_date}"
                )

        # 获取数据库中最新的数据日期
        latest_date = (
            await self._get_latest_date()
        )  # Assume _get_latest_date is fast or add stop_event if needed
        start_date = None
        ...
        if use_trade_days:
            ...
            if lookback_days is not None and lookback_days > 0:
                ...
                try:
                    ...
                    self.logger.info(
                        f"按回溯 {lookback_days} 个交易日计算，起始日期为: {start_date}"
                    )
                except Exception as e:
                ...
                try:
                    ...
                    if next_day and next_day <= end_date:
                        start_date = next_day
                        self.logger.info(
                            f"数据库最新日期为 {latest_date}，从下一个交易日 {start_date} 开始更新"
                        )
                    elif not next_day:
                        self.logger.info(
                            f"无法确定 {latest_date} 的下一个交易日。数据可能已最新。"
                        )
                        return {"status": "up_to_date", "rows": 0}
                    else:  # next_day > end_date
                        self.logger.info(
                            f"数据库最新日期 {latest_date} 之后无新交易日需要更新"
                        )
                        return {"status": "up_to_date", "rows": 0}
                except Exception as e:
                    self.logger.error(
                        f"获取 {latest_date} 的下一个交易日时发生错误: {e}"
                    )
                    return self.handle_error(e)
            else:
                # 数据库无记录，获取最近30个交易日
                try:
                    ...
                    self.logger.info(
                        f"数据库无记录，默认更新最近30个交易日，起始日期: {start_date}"
                    )
                except Exception as e:
        ...
        if start_date and end_date:
            ...
            result = await self.execute(
                stop_event=stop_event, smart_incremental=True, **kwargs
            )
            ...
            # <<< Handle cancellation result from execute >>>
                ...
                self.logger.info(
                    f"增量更新完成: 新增/更新 {result.get('rows', 0)} 行数据"
                )
