...
def _load_db_config() -> Optional[Dict[str, Any]]:
    ...
    if local_app_data:
        user_config_path_parts = [local_app_data, 'trademaster', 'alphahome', 'config.json']
        user_config_path = os.path.join(*user_config_path_parts)
        logger.info(f"_load_db_config: 尝试读取用户特定配置文件: {os.path.abspath(user_config_path)}")
        if os.path.exists(user_config_path):
            actual_path = user_config_path
            logger.info(f"_load_db_config: 实际读取的用户配置文件路径: {os.path.abspath(actual_path)}")
            try:
                with open(actual_path, 'r', encoding='utf-8') as f:
                    config_data = json.load(f)
                db_config = config_data.get("database")
                logger.info(f"_load_db_config: 从 {os.path.abspath(actual_path)} 加载的 database 配置: {db_config}")
                if db_config and isinstance(db_config, dict) and db_config.get("url"):
                    return db_config
                else:
                    logger.warning(f"用户配置文件 {os.path.abspath(actual_path)} 中缺少 'database' 部分或 'url'。")
            except Exception as e:
                logger.error(f"加载或解析用户配置文件 {os.path.abspath(actual_path)} 时出错: {e}")
        else:
            logger.info(f"_load_db_config: 用户特定配置文件未找到: {os.path.abspath(user_config_path)}")
    else:
        logger.warning("_load_db_config: LOCALAPPDATA 环境变量未设置，无法定位用户特定配置文件。")
    ...
    example_config_file_path = os.path.join(current_dir, '..', '..', '..', 'config.example.json')
    
    logger.info(f"_load_db_config: 尝试读取示例配置文件: {os.path.abspath(example_config_file_path)}")
    actual_path = example_config_file_path # 现在 actual_path 指向 example config

    if os.path.exists(actual_path):
        logger.info(f"_load_db_config: 实际读取的示例配置文件路径: {os.path.abspath(actual_path)}")
        try:
            with open(actual_path, 'r', encoding='utf-8') as f:
                config_data = json.load(f)
            db_config = config_data.get("database")
            logger.info(f"_load_db_config: 从 {os.path.abspath(actual_path)} 加载的 database 配置 (回退): {db_config}")
            if db_config and isinstance(db_config, dict) and db_config.get("url"):
                return db_config
            else:
                logger.error(f"回退配置文件 {os.path.abspath(actual_path)} 中缺少 'database' 部分或 'url'。")
                return None
        except Exception as e:
            logger.error(f"加载或解析回退配置文件 {os.path.abspath(actual_path)} 时出错: {e}")
            return None
    else:
...
async def get_trade_cal(start_date: str = None, end_date: str = None, exchange: str = 'SSE') -> pd.DataFrame:
    ...
    """
    if exchange == '':
        logger.warning(f"函数 {__name__}.{inspect.currentframe().f_code.co_name} 收到空的 'exchange' 参数，已自动修正为 'SSE'。")
        exchange = 'SSE'

    logger.info(f"_get_trade_cal: Called with exchange='{exchange}', start_date='{start_date}', end_date='{end_date}'")
    ...
    logger.info(f"_get_trade_cal: Calculated exchange codes: input_exchange='{exchange}', upper_exchange='{upper_exchange}', db_exchange_code='{db_exchange_code}'")
    ...
    try:
        async with pool.acquire() as conn:
            records = await conn.fetch(sql_query, db_exchange_code, start_date, end_date)
        
        logger.info(f"_get_trade_cal: Database raw records count for {db_exchange_code} ({start_date}-{end_date}): {len(records) if records else 'None'}")
        if records:
            logger.debug(f"_get_trade_cal: First raw record (sample): {dict(records[0]) if records else 'N/A'}")
                ...
                for col_name in ['cal_date', 'pretrade_date']:
                    if col_name in final_df.columns:
                        final_df[col_name] = final_df[col_name].apply(
                            lambda x: x.strftime('%Y%m%d') if pd.notnull(x) and isinstance(x, (datetime.date, datetime.datetime)) else None
                        )
                if 'is_open' in final_df.columns: # 确保 is_open 是 Int64 类型以支持 NA
                    final_df['is_open'] = pd.to_numeric(final_df['is_open'], errors='coerce').astype('Int64')
            logger.info(f"从数据库获取并处理了 {len(final_df)} 条 {db_exchange_code} 交易日历数据 ({start_date}-{end_date})。")
        else:
            logger.info(f"数据库未返回 {db_exchange_code} 交易日历数据 ({start_date}-{end_date})。")
            # final_df 已经是一个空的 DataFrame
            
    except Exception as e:
        logger.error(f"从数据库查询交易日历失败 (exchange: {db_exchange_code}, range: {start_date}-{end_date}): {e}")
...
async def is_trade_day(date: Union[str, datetime.datetime, datetime.date], exchange: str = 'SSE') -> bool:
    ...
    calendar_df = await get_trade_cal(start_date=date_str, end_date=date_str, exchange=exchange)
...
async def get_last_trade_day(date: Union[str, datetime.datetime, datetime.date] = None, 
    ...
    calendar_df = await get_trade_cal(start_date=start_search_str, end_date=date_str, exchange=exchange)
    
    if calendar_df.empty:
        logger.warning(f"在 {start_search_str} 和 {date_str} 之间未找到 {exchange} 的交易日历数据.")
        return None
    ...
    trade_days_at_or_before = sorted(
        calendar_df[
            ...
            (calendar_df['cal_date'] <= date_str)
        ]['cal_date'].unique().tolist(), 
        reverse=True
    )

    if not trade_days_at_or_before:
        logger.warning(f"在 {date_str} 或之前未找到 {exchange} 的交易日 (在搜索范围 {start_search_str} 内).")
        return None
    ...
    if target_idx >= 0 and target_idx < len(trade_days_at_or_before):
        ...
        logger.warning(f"没有足够的历史交易日来找到 {date_str} 前的满足条件的第 {n} 个交易日 ({exchange}). 可用: {len(trade_days_at_or_before)}, 目标索引: {target_idx}")
        return None
...
async def get_next_trade_day(date: Union[str, datetime.datetime, datetime.date] = None, 
    ...
    calendar_df = await get_trade_cal(start_date=start_search_str, end_date=end_search_str, exchange=exchange)

    if calendar_df.empty:
        logger.warning(f"在 {start_search_str} 和 {end_search_str} 之间未找到 {exchange} 的交易日历数据.")
        return None
    ...
    trade_days_on_or_after = sorted(
        calendar_df[
            (calendar_df['is_open'] == 1) &
            (calendar_df['cal_date'] >= start_search_str) # 修正：这里之前错误地使用了 date_str，应为 start_search_str
        ]['cal_date'].unique().tolist()
    )

    if not trade_days_on_or_after:
        logger.warning(f"在 {start_search_str} 或之后未找到 {exchange} 的交易日 (在搜索范围直到 {end_search_str} 内).")
        return None
    ...
    if target_idx >= 0 and target_idx < len(trade_days_on_or_after):
        ...
        logger.warning(f"没有足够的未来交易日来找到 {start_search_str} 后的满足条件的第 {n} 个交易日 ({exchange}). 可用: {len(trade_days_on_or_after)}, 目标索引: {target_idx}")
        return None
...
async def get_trade_days_between(start_date: Union[str, datetime.datetime, datetime.date],
    ...
    try:
        ...
        logger.error(f"get_trade_days_between 的日期格式无效: {start_date_str} 或 {end_date_str}. 需要 YYYYMMDD.")
        return []

    if start_date_str > end_date_str:
        logger.warning(f"开始日期 {start_date_str} 在结束日期 {end_date_str} 之后，将返回空列表。")
        return []

    calendar_df = await get_trade_cal(start_date=start_date_str, end_date=end_date_str, exchange=exchange)
