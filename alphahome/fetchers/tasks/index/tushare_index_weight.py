...
from pandas.tseries.offsets import YearEnd, YearBegin # 需要导入
...
class TushareIndexWeightTask(TushareTask):
    ...
    default_start_date = "20050101" # 如果数据库中无数据，默认的起始日期，可根据需要调整

    # --- 代码级别的默认配置 (可被 config.json 文件覆盖) ---
    default_concurrent_limit = 10
    default_page_size = 8000 # TushareAPI 处理分页；index_weight 是月度数据，此限制通常不触发
    ...
    def _adjust_dates_for_monthly_api(self, start_date_str: Optional[str], end_date_str: Optional[str]) -> tuple[Optional[str], Optional[str]]:
        ...
        try:
            ...
            if adj_start_dt > adj_end_dt:
                 ...
                 adj_end_dt = (start_dt.replace(day=1) + pd.DateOffset(months=1)) - pd.DateOffset(days=1)
    ...
    def _calculate_incremental_dates(self, latest_db_date_str: Optional[str]) -> tuple[Optional[str], str]:
        """
        计算增量更新的原始开始和结束日期。
        ...
        if latest_db_date_str:
            try:
                start_dt = pd.to_datetime(latest_db_date_str, format='%Y%m%d') + pd.Timedelta(days=1)
            ...
            self.logger.info(f"数据库中未找到任务 {self.task_id} 的最新日期。使用默认起始日期: {current_default_start}")
            effective_start_date_str = current_default_start
    ...
    async def get_index_codes(self) -> List[str]:
        ...
        """
        self.logger.debug(f"任务 {self.name} 正在从 'index_basic' API 获取所有指数代码。") # 使用 self.name
        try:
            if not self.api:
                self.logger.error("TushareAPI 实例 (self.api) 不可用。")
                return []
            
            df_codes = await self.api.query(api_name="index_basic", fields=["ts_code"]) # ts_code 是 index_basic 中的字段
            ...
            if df_codes is not None and not df_codes.empty:
                ...
                self.logger.warning("从 'index_basic' API 返回的指数代码为空或DataFrame为空。")
                return []
        except Exception as e:
            self.logger.error(f"获取任务 {self.name} 的指数代码时出错: {e}", exc_info=True)
            return []
    ...
    async def get_batch_list(self, **kwargs: Any) -> List[Dict[str, Any]]:
        ...
        self.logger.info(f"任务 {self.name}: 开始为总日期范围 {overall_start_date_str}-{overall_end_date_str} 生成批处理列表 (每批 {self.date_split_interval_years} 年)。")

        if not overall_start_date_str or not overall_end_date_str:
            self.logger.error(f"任务 {self.name}: get_batch_list 需要总的 start_date 和 end_date。接收到 start='{overall_start_date_str}', end='{overall_end_date_str}'")
            return []
        ...
        try:
            overall_start_dt = pd.to_datetime(overall_start_date_str, format=date_format)
            overall_end_dt = pd.to_datetime(overall_end_date_str, format=date_format)
        except ValueError as e:
            self.logger.error(f"任务 {self.name}: 解析总日期范围 ({overall_start_date_str} - {overall_end_date_str}) 失败: {e}")
            return []
        ...
        for index_code in index_codes:
            ...
            while current_period_start_dt <= overall_end_dt:
                ...
                current_period_end_dt = pd.Timestamp(year=period_end_year, month=12, day=31)
                ...
                next_period_start_dt = pd.Timestamp(year=current_period_end_dt.year + 1, month=1, day=1)
                
                # 如果下一个周期的开始已经晚于总结束日期，则停止为当前 index_code 生成批次
                if next_period_start_dt > overall_end_dt and current_period_end_dt >= overall_end_dt:
                    break
    ...
    async def smart_incremental_update(self, **kwargs: Any) -> Dict[str, Any]:
        ...
        effective_start_date_str, effective_end_date_str = self._calculate_incremental_dates(latest_db_date_str)

        if not effective_start_date_str:
            self.logger.warning(f"任务 {self.name}: 未能计算出有效的开始日期用于智能增量。")
            return { "task_id": self.name, "status": "failed", "message": "未能计算有效开始日期"}
        ...
        if start_dt_obj_check > end_dt_obj_check:
            ...
            return {
                "task_id": self.name, "status": "no_new_data", "api": self.api_name, 
