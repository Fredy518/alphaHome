...
class TushareFundPortfolioTask(TushareTask):
    ...
    transformations = {
        ...
        "stk_float_ratio": lambda x: pd.to_numeric(x, errors='coerce') # 占流通股本比例
    ...
    async def get_batch_list(self, **kwargs: Any) -> List[Dict]:
        ...
        if not start_date:
            ...
            self.logger.info(f"未提供 start_date，使用数据库最新公告日期+1天或默认起始日期: {start_date}")
        if not end_date:
        ...
        self.logger.info(f"任务 {self.name}: 生成批处理列表 (基于 ann_date)，范围: {start_date} 到 {end_date}, 代码: {ts_code if ts_code else '所有'}")
