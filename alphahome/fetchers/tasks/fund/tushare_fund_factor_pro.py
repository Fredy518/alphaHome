...
"""
场内基金技术因子(专业版) (fund_factor_pro) 数据任务

接口文档: https://tushare.pro/document/2?doc_id=222
数据说明:
- 获取场内基金每日技术面因子数据，用于跟踪场内基金当前走势情况
...
@task_register()
class TushareFundFactorProTask(TushareTask):
    """获取场内基金技术因子数据 (fund_factor_pro)"""

    # 1. 核心属性
    name = "tushare_fund_factor_pro"
    description = "获取场内基金每日技术面因子数据"
    table_name = "fund_factor_pro"
    primary_keys = ["ts_code", "trade_date"]
    date_column = "trade_date"
    default_start_date = "19991020"
    data_source = "tushare"
    domain = "fund"
    ...
    # 6. 自定义索引
    indexes = [
        {"name": "idx_fund_factor_pro_ts_code", "columns": "ts_code"},
        {"name": "idx_fund_factor_pro_trade_date", "columns": "trade_date"},
        {"name": "idx_fund_factor_pro_update_time", "columns": "update_time"},
    ]
    ...
    async def get_batch_list(self, **kwargs: Any) -> List[Dict]:
        """
        生成批处理参数列表。
        - 全量模式：按 ts_code 分批，从 fund_basic 表获取场内基金代码
    ...
    async def _get_full_batch_list(self, **kwargs: Any) -> List[Dict]:
        """全量模式：从 fund_basic 表获取场内基金代码（SH/SZ后缀），按 ts_code 分批"""
        self.logger.info(f"任务 {self.name}: 全量模式，按基金代码分批")

        # 从数据库获取场内基金代码
        query = """
            SELECT ts_code FROM tushare.fund_basic 
        ...
        try:
            ...
            self.logger.info(f"从 fund_basic 获取到 {len(ts_codes)} 个场内基金代码")
        except Exception as e:
            self.logger.error(f"获取场内基金代码失败: {e}")
            return []

        if not ts_codes:
            self.logger.warning("未找到场内基金代码，请先执行 tushare_fund_basic 任务")
            return []
    ...
    async def _get_incremental_batch_list(self, **kwargs: Any) -> List[Dict]:
        ...
        self.logger.info(f"任务 {self.name}: 增量模式，按交易日分批 {start_date} ~ {end_date}")
...
__all__ = ["TushareFundFactorProTask"]
