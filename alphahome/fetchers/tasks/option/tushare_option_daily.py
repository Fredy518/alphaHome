...
class TushareOptionDailyTask(TushareTask):
    ...
    async def get_batch_list(self, **kwargs: Any) -> List[Dict]:
        ...
        all_batches = []
        self.logger.info(f"任务 {self.name}: 按交易所和自然日生成批处理列表，范围: {start_date} 到 {end_date}")

        try:
            for exchange in self.option_exchanges:
                self.logger.info(f"任务 {self.name}: 为交易所 {exchange} 生成自然日批次...")
                # 使用自然日批次工具函数
                ...
                self.logger.info(f"任务 {self.name}: 交易所 {exchange} 生成了 {len(exchange_batches)} 个批次。")

            self.logger.info(f"任务 {self.name}: 成功生成总计 {len(all_batches)} 个批次。")
            return all_batches
    ...
    async def process_data(self, df: pd.DataFrame, **kwargs) -> pd.DataFrame:
        ...
        self.logger.info(f"任务 {self.name}: process_data 被调用，返回 DataFrame (行数: {len(df)}). 基类处理已完成。")
        return df
    ...
    async def validate_data(self, df: pd.DataFrame, **kwargs: Any) -> pd.DataFrame:
        ...
        # 关键业务字段列表
        critical_cols = ['ts_code', 'trade_date', 'exchange', 'close', 'volume'] # 使用映射后的 volume
