...
class TushareOptionBasicTask(TushareTask):
    ...
    fields = [
        'ts_code', 'exchange', 'name', 'per_unit', 'opt_code', 'opt_type', 'call_put', 
        'exercise_type', 'exercise_price', 's_month', 'maturity_date', 'list_price', 
        'list_date', 'delist_date', 'last_edate', 'last_ddate', 'quote_unit', 'min_price_chg'
    ]
    ...
    schema = {
        ...
        "per_unit": {"type": "FLOAT"}, # 文档是str，暂定FLOAT，若API返回str需调整或在process_data处理
    ...
    async def validate_data(self, df: pd.DataFrame, **kwargs: Any) -> pd.DataFrame:
        ...
        # 关键业务字段列表
        critical_cols = ['ts_code', 'name', 'exchange', 'call_put', 'exercise_price', 'maturity_date']
        ...
        if df['exercise_price'].isnull().all() or df['maturity_date'].isnull().all():
            self.logger.warning(f"任务 {self.name}: 数据验证警告 - exercise_price 或 maturity_date 列全为空，请检查数据源或API调用。")
