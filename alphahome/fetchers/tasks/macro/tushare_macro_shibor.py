...
class TushareMacroShiborTask(TushareTask):
    ...
    async def get_batch_list(self, **kwargs: Any) -> List[Dict]:
        ...
        self.logger.info(f"任务 {self.name}: 使用自然日按年生成批处理列表，范围: {start_date} 到 {end_date}")
    ...
    async def process_data(self, df: pd.DataFrame, **kwargs) -> pd.DataFrame:
        ...
        # 此处可以添加 shibor 特有的额外处理，如果需要的话。

        self.logger.info(f"任务 {self.name}: process_data 被调用，返回 DataFrame (行数: {len(df)})。")
        return df
