...
class TushareMacroCpiTask(TushareTask):
    ...
    schema = {
        "month": {"type": "VARCHAR(10)", "constraints": "NOT NULL"},  # YYYYMM 格式, 保留原始月份
    ...
    indexes = [
        ...
        {"name": "idx_cpi_month_orig", "columns": "month"}, # 可选：为原始 month 列保留索引
    ...
    async def get_batch_list(self, **kwargs: Any) -> List[Dict]:
        ...
        self.logger.info(f"任务 {self.name}: 使用月份批次生成批处理列表，范围: {start_date} 到 {end_date}")
        ...
        try:
            ...
            self.logger.info(f"任务 {self.name}: 成功生成 {len(batch_list)} 个月度批次。")
            return batch_list
        except Exception as e:
            self.logger.error(f"任务 {self.name}: 生成月度批次时出错: {e}", exc_info=True)
            # 抛出异常以便上层调用者感知
    ...
    async def process_data(self, df: pd.DataFrame, **kwargs) -> pd.DataFrame:
        ...
        try:
            df['month_end_date'] = pd.to_datetime(df['month'], format='%Y%m') + pd.offsets.MonthEnd(0)
            # 确保转换为纯日期对象，如果数据库列是 DATE 类型
            df['month_end_date'] = df['month_end_date'].dt.date
        except Exception as e:
            self.logger.error(f"任务 {self.name}: 计算 month_end_date 时出错: {e}", exc_info=True)
            raise ValueError(f"任务 {self.name}: month_end_date 计算失败，停止处理。") from e

        self.logger.info(f"任务 {self.name}: process_data 被调用，已添加 month_end_date，返回 DataFrame (行数: {len(df)})。")
        return df
    ...
    async def validate_data(self, df: pd.DataFrame, **kwargs: Any) -> pd.DataFrame:
        ...
        critical_cols = ['month', 'nt_val', 'nt_yoy']  # 最关键的字段：月份、全国当月值和同比
