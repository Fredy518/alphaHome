...
class TushareFutureDailyTask(TushareTask):
    ...
    fields = [
        ...
        'pre_close', 'settle', 'pre_settle', 'zd_1d', 'zd_dt',
    ...
    async def get_batch_list(self, **kwargs: Any) -> List[Dict]:
        ...
        self.logger.info(f"任务 {self.name}: 使用自然日按月生成批处理列表，范围: {start_date} 到 {end_date}")
        ...
        try:
            ...
            self.logger.info(f"任务 {self.name}: 成功生成 {len(batch_list)} 个自然日批次。")
            return batch_list
    ...
    async def validate_data(self, df: pd.DataFrame, **kwargs: Any) -> pd.DataFrame:
        ...
        critical_cols = ['ts_code', 'trade_date', 'close', 'volume'] # 使用映射后的 volume
