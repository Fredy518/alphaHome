...
class TushareFutureHoldingTask(TushareTask):
    ...
    default_start_date = "20100416" # 可根据实际期货市场数据最早日期调整【中金所最早数据】
    ...
    SUPPORTED_EXCHANGES = ['CFFEX'] # 暂时仅保存中金所数据; 其他交易所代码:'DCE', 'CZCE', 'SHFE', 'INE', 'GFEX'
    ...
    schema = {
        ...
        "symbol": {"type": "VARCHAR(30)", "constraints": "NOT NULL"}, # 合约代码，可能包含交易所后缀或特定产品代码
    ...
    async def get_batch_list(self, **kwargs: Any) -> List[Dict]:
        ...
        self.logger.info(f"任务 {self.name}: 为交易所列表 {self.SUPPORTED_EXCHANGES} 生成单交易日批次，范围: {start_date} 到 {end_date}")
        ...
        for ex_code in self.SUPPORTED_EXCHANGES:
            try:
                ...
                batches_for_exchange = await generate_single_date_batches(
                    ...
                    additional_params={"exchange": ex_code}, # 此 exchange 参数会传递给 Tushare API
                    logger=self.logger
                )
                if batches_for_exchange:
                    all_batches.extend(batches_for_exchange)
                    self.logger.info(f"任务 {self.name}: 为交易所 {ex_code} 生成了 {len(batches_for_exchange)} 个批次。")
                else:
                    self.logger.info(f"任务 {self.name}: 交易所 {ex_code} 在指定日期范围 {start_date}-{end_date} 内无批次生成（可能无交易日）。")
            except Exception as e:
                self.logger.error(f"任务 {self.name}: 为交易所 {ex_code} 生成批次时出错: {e}", exc_info=True)
    ...
    async def process_data(self, df: pd.DataFrame, **kwargs: Any) -> pd.DataFrame:
        ...
        if not isinstance(df, pd.DataFrame) or df.empty:
            self.logger.info(f"任务 {self.name}: process_data 接收到空 DataFrame，跳过处理。")
            return df
        ...
        self.logger.info(f"任务 {self.name}: process_data 处理 DataFrame (行数: {len(df)}).")
        return df
    ...
    async def validate_data(self, df: pd.DataFrame, **kwargs: Any) -> pd.DataFrame:
        ...
        for pk_col in self.primary_keys:
            if df_check[pk_col].isnull().any():
                error_msg = f"任务 {self.name}: 数据验证失败 - 主键字段 '{pk_col}' 包含空值。"
                self.logger.error(error_msg)
