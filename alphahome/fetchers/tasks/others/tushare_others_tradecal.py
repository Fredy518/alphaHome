...
class TushareOthersTradecalTask(TushareTask):
    ...
    _EXCHANGES_TO_FETCH: List[str] = ['SSE', 'SZSE', 'CFFEX', 'SHFE', 'CZCE', 'DCE', 'INE']
    ...
    async def get_batch_list(self, start_date: str, end_date: str, **kwargs: Any) -> List[Dict]:
        ...
        self.logger.info(f"任务 {self.name}: 生成 {len(batches)} 个批次。全局日期范围: {start_date} - {end_date}")
        return batches
        ...
        for col_name, func in self.transformations.items():
            if col_name in df.columns:
                try:
                    ...
                    self.logger.warning(f"任务 {self.name}: 对列 '{col_name}' 应用转换失败: {e}") # ({exchange_log})
    ...
    async def validate_data(self, df: pd.DataFrame, **kwargs: Any) -> pd.DataFrame:
        ...
        for pk_col in self.primary_keys:
            if df[pk_col].isnull().any():
                error_msg = f"任务 {self.name} ({exchange_log}): 主键列 '{pk_col}' 含空值。"
                self.logger.error(error_msg)
                raise ValueError(error_msg)
        ...
        if 'is_open' in df.columns:
            ...
            if not valid_is_open_values.empty:
                if not valid_is_open_values.isin([0, 1]).all():
                    self.logger.warning(f"任务 {self.name} ({exchange_log}): 列 'is_open' 含意外值。")
