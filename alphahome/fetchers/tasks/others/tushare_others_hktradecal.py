...
class TushareOthersHktradecalTask(TushareTask):
    ...
    table_name: str = "tushare_others_calendar"  # 与大陆任务共享同一个表
    ...
    fields: List[str] = ['exchange', 'cal_date', 'is_open', 'pretrade_date'] # 目标数据库字段
    # 根据用户之前反馈，假设hk_tradecal直接返回pretrade_date，故无需映射pre_trade_date
    column_mapping: Dict[str, str] = {}
    transformations: Dict[str, Any] = {
        "is_open": lambda x: pd.to_numeric(x, errors='coerce') # 'Series' object has no attribute 'as_type'
    }
    ...
    def __init__(self, db_connection, api_token: Optional[str] = None, api: Optional[Any] = None):
        """初始化 TushareOthersHktradecalTask."""
        super().__init__(db_connection, api_token=api_token, api=api)
        self.logger.info(f"任务 {self.name} 初始化完成。将从 {self.default_start_date} 开始获取数据。")

    async def get_batch_list(self, start_date: str, end_date: str, **kwargs: Any) -> List[Dict]:
        """使用自然日分块生成批次，每批2000天。"""
        batches = await generate_natural_day_batches(start_date, end_date, batch_size=2000, logger=self.logger)
        self.logger.info(f"任务 {self.name}: 生成 {len(batches)} 个自然日批次 (每批2000天)。全局日期范围: {start_date} - {end_date}")
        return batches

    async def fetch_data_for_batch(self, batch_params: Dict, start_date: str, end_date: str) -> pd.DataFrame:
        """直接用批次参数的 start_date 和 end_date 调用 Tushare API。"""
        self.logger.info(f"任务 {self.name}: 获取数据，日期范围: {start_date} - {end_date}")
        ...
        try:
            ...
            if df is None:
                self.logger.warning(f"任务 {self.name}: API调用返回 None，参数: {params}")
                return pd.DataFrame()
            if df.empty:
            ...
            self.logger.error(f"任务 {self.name}: 调用 {self.api_name} 时发生错误，参数 {params}: {e}")
            return pd.DataFrame()
        ...
        for col_name, func in self.transformations.items():
            if col_name in df.columns:
                try:
                    ...
                    self.logger.warning(f"任务 {self.name}: 对列 '{col_name}' 应用转换失败: {e}")
        ...
        if 'is_open' in df.columns:
            ...
            if not valid_is_open_values.empty:
                if not valid_is_open_values.isin([0, 1]).all():
                    self.logger.warning(f"任务 {self.name} (HKEX): 列 'is_open' 包含意外值 (非0或1)。")
        return df 