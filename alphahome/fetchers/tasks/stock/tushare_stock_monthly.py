...
@task_register()
class TushareStockMonthlyTask(TushareTask):
    """获取股票月线行情数据 (stk_weekly_monthly)
    """

    # 1. 核心属性
    name = "tushare_stock_monthly"
    ...
    data_source = "tushare"
    domain = "stock"  # 业务域标识
    smart_lookback_days = 60  # 智能增量模式下，回看60天
    ...
    # 6. 自定义索引
    indexes = [
        {"name": "idx_stock_monthly_ts_code", "columns": "ts_code"},
        {"name": "idx_stock_monthly_trade_date", "columns": "trade_date"},
        {"name": "idx_stock_monthly_update_time", "columns": "update_time"},
    ]
    ...
    async def get_batch_list(self, **kwargs) -> List[Dict]:
        ...
        try:
            ...
            if update_type in (UpdateTypes.FULL, UpdateTypes.SMART):
                ...
                # 计算当前月的最后一天
                next_month = current_date.replace(day=28) + timedelta(days=4)  # 跳到下个月
                month_end = next_month - timedelta(days=next_month.day)  # 回到本月最后一天
                normalized_end = month_end.strftime("%Y%m%d")
                self.logger.info(f"任务 {self.name}: {update_type}模式下end_date对齐为当前月末: {normalized_end}")
            else:
            ...
            # 使用 generate_natural_day_batches 生成批次，每批120个自然日
            return await generate_natural_day_batches(
                start_date=normalized_start,
                end_date=normalized_end,
                batch_size=120,  # 每批120个自然日
                additional_params={
...
# 导出任务类
__all__ = ["TushareStockMonthlyTask"]
