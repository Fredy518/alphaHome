...
from ...tools.batch_utils import generate_single_date_batches # 改回使用 generate_single_date_batches
...
class TushareStockChipsTask(TushareTask):
    ...
    # 6. 表结构定义 (包含注释)
    schema = {
        "ts_code": {"type": "VARCHAR(10)", "constraints": "NOT NULL", "comment": "股票代码"},
        "trade_date": {"type": "DATE", "constraints": "NOT NULL", "comment": "交易日期"},
    ...
    async def get_batch_list(self, **kwargs: Any) -> List[Dict[str, Any]]:
        ...
        if not start_date_overall:
            ...
            self.logger.info(f"任务 {self.name}: 未提供 start_date，使用数据库最新日期+1天或默认起始日期: {start_date_overall}")

        if not end_date_overall:
            end_date_overall = datetime.now().strftime('%Y%m%d')
            self.logger.info(f"任务 {self.name}: 未提供 end_date，使用当前日期: {end_date_overall}")

        if pd.to_datetime(start_date_overall) > pd.to_datetime(end_date_overall):
            self.logger.info(f"任务 {self.name}: 起始日期 ({start_date_overall}) 晚于结束日期 ({end_date_overall})，无需执行任务。")
            return []
        ...
        try:
            ...
            self.logger.info(f"任务 {self.name}: 成功生成 {len(batch_list)} 个单日期批次。")
            return batch_list
    ...
    async def post_execute(self, result: Dict[str, Any], stop_event: Optional[asyncio.Event] = None, **kwargs):
        """任务执行后的清理工作"""
