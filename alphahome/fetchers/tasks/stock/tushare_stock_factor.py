...
class TushareStockFactorProTask(TushareTask):
    ...
    schema_def = {
        ...
        "volume": {"type": "NUMERIC(18,2)"},  # 目标字段名
        # 动态生成其他所有数值字段的 schema
        # 注意：这里排除的是API返回的原始字段名，但volume字段已经单独定义
        **{
            col: {"type": "NUMERIC(18, 4)"}
            for col in fields
            if col not in ["ts_code", "trade_date", "vol", "downdays", "updays", "lowdays", "topdays"]
        },
