...
class TinySoftStockFinaPitExtTask(TinySoftTask):
    ...
    default_smart_symbol_batch_size = 120
    ...
    async def fetch_batch(self, params: Dict[str, Any], stop_event=None) -> Optional[pd.DataFrame]:
        ...
        combined = await self.fetch_infotable_for_symbol_pairs(
            table_id=table_id,
            symbol_pairs=symbol_pairs,
            ...
            error_label="Tinysoft 财务",
        )
        if combined is None or combined.empty:
            return None

        result = combined.copy()
        result["finance_source"] = finance_source
        result["source_table_id"] = table_id
        result["metric_defs"] = [metric_defs] * len(result)
        return result
