...
class TushareAPI:
    ...
    def __init__(self, token=None, logger=None):
        ...
        # 合并已知API列表，避免重复
        all_known_apis = set(self._api_max_requests_per_minute.keys()) | set(self._api_concurrency_limits.keys())

        for api_name in all_known_apis:
            # 初始化信号量
            if api_name not in TushareAPI._api_semaphores:
                limit = self._api_concurrency_limits.get(api_name, self._default_concurrency_limit)
                TushareAPI._api_semaphores[api_name] = asyncio.Semaphore(limit)
                if self.logger:
                    self.logger.debug(f"为 API {api_name} 创建并发信号量，限制: {limit}")
    ...
    async def _wait_for_rate_limit_slot(self, api_name: str):
        ...
        if api_name not in TushareAPI._api_request_timestamps:
            ...
            TushareAPI._api_request_timestamps[api_name] = collections.deque()
            if self.logger:
                self.logger.debug(f"动态为 API {api_name} 创建速率控制时间戳队列 (使用默认速率)")
        
        timestamps_deque = TushareAPI._api_request_timestamps[api_name]
        limit_per_window = self._api_max_requests_per_minute.get(api_name, self._default_max_requests_per_minute)

        while True:
            current_time = time.monotonic()
            
            while timestamps_deque and timestamps_deque[0] <= current_time - self._rate_limit_window_seconds:
                timestamps_deque.popleft()
            
            if len(timestamps_deque) < limit_per_window:
                timestamps_deque.append(current_time)
                self.logger.debug(f"速率控制 ({api_name}): 允许请求。窗口内 {self._rate_limit_window_seconds}s 请求数: {len(timestamps_deque)}/{limit_per_window}")
                break
            else:
                time_to_wait = (timestamps_deque[0] + self._rate_limit_window_seconds) - current_time + 0.01 
                if time_to_wait <= 0: time_to_wait = 0.05 # 最小等待时间，避免潜在的CPU空转
                
                self.logger.debug(f"速率控制 ({api_name}): 超出限制 ({len(timestamps_deque)}/{limit_per_window})。等待 {time_to_wait:.2f} 秒... (队列首位时间戳: {timestamps_deque[0] if timestamps_deque else 'N/A'})")
    ...
    def _get_semaphore_for_api(self, api_name: str) -> asyncio.Semaphore:
        ...
        if api_name not in TushareAPI._api_semaphores:
            ...
            TushareAPI._api_semaphores[api_name] = asyncio.Semaphore(limit)
            if self.logger:
                self.logger.debug(f"动态为 API {api_name} 创建并发信号量，限制: {limit}")
    ...
    async def query(self, api_name: str, params: Dict = None, fields: List[str] = None, page_size: int = None) -> pd.DataFrame:
        ...
        # 确保 page_size 有一个值
        effective_page_size = page_size if page_size is not None and page_size > 0 else 5000
        ...
        while has_more:
            ...
            current_semaphore = self._get_semaphore_for_api(api_name) # 获取/创建当前API的信号量
            
            # 使用信号量控制并发
            async with current_semaphore:
                if self.logger: # 避免在非debug模式下过于频繁的日志
                    self.logger.debug(f"并发控制 ({api_name}): 获取 Semaphore 许可 (当前并发上限: {current_semaphore._value if hasattr(current_semaphore, '_value') else 'N/A'})")
                ...
                try:
                    ...
                    if offset > 0:
                        self.logger.info(f"分页请求 ({api_name}): offset={offset}, limit={page_params['limit']}, params={params}")
                    else:
                        self.logger.debug(f"首次请求 ({api_name}): offset={offset}, limit={page_params['limit']}, params={params}")

                    payload = {
                        "api_name": api_name, "token": self.token,
                        "params": page_params, "fields": fields or ""
                    }
                    ...
                    async with aiohttp.ClientSession() as session:
                        async with session.post(self.url, json=payload) as response:
                            if response.status != 200:
                                ...
                                self.logger.error(f"Tushare API 请求失败 ({api_name}): 状态码: {response.status}, URL: {self.url}, Payload: {payload}, 响应: {error_text}")
                                raise ValueError(f"Tushare API 请求失败({api_name})，状态码: {response.status}, 响应: {error_text}")
                            
                            result = await response.json()
                            if result.get('code') != 0:
                                error_msg = result.get('msg', '未知错误')
                                self.logger.error(f"Tushare API 返回错误 ({api_name}): Code: {result.get('code')}, Msg: {error_msg}, Payload: {payload}")
                                raise ValueError(f"Tushare API 返回错误 ({api_name}): Code: {result.get('code')}, Msg: {error_msg}")
                            
                            data = result.get('data', {})
                            if not data: break # 无数据则终止分页
                            ...
                            if not items: # 如果本次分页未获取到任何条目
                                ...
                                if not all_data and consecutive_empty_pages >= 1: # 如果一开始就没数据，且已尝试1次以上空页
                                    self.logger.info(f"({api_name}) 首次/早期分页即连续 {consecutive_empty_pages} 次返回空数据，提前结束分页。Params: {params}")
                                    has_more = False # 强制结束
                                elif consecutive_empty_pages >= max_consecutive_empty_before_stop:
                                    self.logger.info(f"({api_name}) 连续 {consecutive_empty_pages} 次分页返回空数据，提前结束分页。Offset: {offset}, Params: {params}")
                                    has_more = False # 强制结束
                                ...
                                all_data.append(df)
                                self.logger.debug(f"({api_name}) 本次分页获取 {len(items)} 条记录. Offset: {offset}, PageSize requested: {page_params['limit']}")
                    ...
                    if self.logger: # 避免在非debug模式下过于频繁的日志
                         self.logger.debug(f"并发控制 ({api_name}): 已释放 Semaphore 许可")
        
        if not all_data: return pd.DataFrame()
        combined_data = pd.concat(all_data, ignore_index=True)
        self.logger.info(f"API {api_name} (参数: {params}) 通过分页共获取 {len(combined_data)} 条记录。")
