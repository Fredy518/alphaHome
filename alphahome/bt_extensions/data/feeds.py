...
from ..utils.exceptions import DataError, BacktestError
...
class PostgreSQLDataFeed(bt.feeds.DataBase):
    ...
    def __init__(self, **kwargs):
        ...
        logger.info(f"初始化PostgreSQL数据源: {self.p.ts_code}, 表: {self.p.table_name}")
            ...
            for attempt in range(max_retries):
                try:
                    ...
                    if attempt == max_retries - 1:
                        raise retry_e
                    logger.warning(f"数据库查询重试 {attempt + 1}/{max_retries}: {retry_e}")
                    # 短暂等待后重试
                    await asyncio.sleep(0.5)
            
            if not records:
                raise DataError(f"未找到数据: {self.p.ts_code}", 
                              table_name=self.p.table_name,
                              ts_code=self.p.ts_code)
...
class PostgreSQLDataFeedFactory:
    ...
    def create_feed(self, ts_code: str, table_name: str = 'tushare_stock_daily',
                   start_date: Optional[date] = None, 
                   end_date: Optional[date] = None,
                   **kwargs) -> PostgreSQLDataFeed:
        """
        创建数据源实例
    ...
    async def get_available_codes(self, table_name: str = 'tushare_stock_daily') -> List[str]:
        """
        异步获取可用的股票代码列表
