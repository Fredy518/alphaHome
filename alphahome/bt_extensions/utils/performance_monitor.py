...
class PerformanceMonitor:
    ...
    def _generate_stats(self) -> Dict[str, Any]:
        ...
        if self._memory_samples:
            ...
            stats["memory"] = {
                "initial_rss_mb": (
                    self._initial_memory.rss / 1024 / 1024
                ...
                "memory_growth_mb": (
                    (rss_values[-1] - rss_values[0]) / 1024 / 1024
                    if len(rss_values) > 1
                    else 0
                ),
            }
        ...
        if self._io_samples and self._initial_io:
            ...
            stats["io"] = {
                ...
                "avg_read_speed_mbps": (
                    (total_read_bytes / 1024 / 1024) / duration if duration > 0 else 0
                ),
                "avg_write_speed_mbps": (
                    (total_write_bytes / 1024 / 1024) / duration if duration > 0 else 0
                ),
            }
    ...
    def print_stats(self, stats: Optional[Dict[str, Any]] = None):
        ...
        if "io" in stats:
            ...
            print(
                f"  写入: {io['total_write_mb']:.1f} MB ({io['total_write_count']} 次)"
            )
        ...
        if "cpu_percent" in stats:
            ...
            print(
                f"  内存: {stats['memory_rss_mb']:.1f} MB ({stats['memory_percent']:.1f}%)"
            )
