...
class EnhancedAnalyzer(bt.Analyzer):
    ...
    def _calculate_basic_stats(self) -> Dict[str, Any]:
        """计算基础统计信息"""
        total_return = ((self.end_value / self.start_value) - 1) * 100 if self.start_value > 0 else 0
    ...
    def _calculate_risk_metrics(self) -> Dict[str, Any]:
        ...
        cvar_5 = np.mean([r for r in returns if r <= var_5]) if returns and var_5 != 0 else 0
    ...
    def _calculate_trade_stats(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'largest_win': max([t['pnl'] for t in self.trade_records]) if self.trade_records else 0,
            'largest_loss': min([t['pnl'] for t in self.trade_records]) if self.trade_records else 0
        }
    ...
    def _calculate_equity_curve_stats(self) -> Dict[str, Any]:
        ...
        return {
            'current_ma_20': ma_20,
            'trend_slope': slope,
            'above_ma_days': sum(1 for v in values[-20:] if v > ma_20) if len(values) >= 20 else 0,
    ...
    def _calculate_performance_metrics(self) -> Dict[str, Any]:
        ...
        # 综合评分 (0-100)
        score_components = {
            'return_score': min(max(basic.get('total_return', 0) / 50 * 25, 0), 25),  # 最高25分
            'risk_score': min(max((100 - risk.get('max_drawdown', 100)) / 100 * 25, 0), 25),  # 最高25分
            'consistency_score': min(max(trades.get('win_rate', 0) / 100 * 25, 0), 25),  # 最高25分
            'efficiency_score': min(max(risk.get('sharpe_ratio', 0) / 2 * 25, 0), 25)  # 最高25分
        }
    ...
    def to_dataframe(self) -> Dict[str, pd.DataFrame]:
        ...
        if self.daily_returns:
            ...
            result['daily_returns']['date'] = pd.to_datetime(result['daily_returns']['date'])
        ...
        if self.trade_records:
            ...
            result['trades']['open_date'] = pd.to_datetime(result['trades']['open_date'])
            result['trades']['close_date'] = pd.to_datetime(result['trades']['close_date'])
