...
class BatchDataLoader:
    ...
    def load_stocks_data(self, 
        ...
        # 2. 批量查询未缓存的数据
        if uncached_codes:
            batch_data = self._batch_query(uncached_codes, start_date, end_date, table_name)
    ...
    def _batch_query(self, 
                    ...
                    start_date: date, 
                    end_date: date, 
                    table_name: str) -> Dict[str, pd.DataFrame]:
        """
        执行批量SQL查询
        """
        if not stock_codes:
    ...
    def _validate_data(self, df: pd.DataFrame):
        ...
        # 检查OHLC数据的合理性
        invalid_ohlc = ~((df['low'] <= df['open']) & (df['open'] <= df['high']) & 
    ...
    def _get_cache_key(self, ts_code: str, start_date: date, end_date: date, table_name: str) -> str:
        """
        生成缓存键
        """
    ...
    def preload_market_data(self, 
                           ...
                           end_date: date,
                           batch_size: int = 100,
                           table_name: str = 'tushare_stock_daily') -> Dict[str, pd.DataFrame]:
        """
        预加载市场数据（分批处理大量股票）
        ...
        Returns:
            所有股票的数据字典
        """
        self.logger.info(f"预加载 {len(stock_codes)} 只股票数据，批次大小: {batch_size}")
        ...
        for i in range(0, len(stock_codes), batch_size):
            ...
            self.logger.info(f"处理批次 {batch_num}/{total_batches}: {len(batch_codes)} 只股票")
