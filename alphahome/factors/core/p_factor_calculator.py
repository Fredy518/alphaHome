...
class PFactorCalculator:
    ...
    def __init__(
        ...
        Args:
            ...
            config: Reserved for compatibility with engine construction.
        ...
        self.data_repository = PFactorDataRepository(self.context, self.logger)
    ...
    def calculate_p_factors_pit(
        ...
        p_factors = self.compute_p_factors_pit(as_of_date, stock_codes)
        ...
        if not p_factors.empty:
            ...
            success_count, checksum = self._save_p_factors_mvp(p_factors)
    ...
    def compute_p_factors_pit(
        self,
        as_of_date: str,
        stock_codes: List[str],
    ) -> pd.DataFrame:
        """Read and calculate one P snapshot without mutating factor tables."""
        query_start = time.time()
        indicators_data = self._get_mvp_precomputed_indicators_pit(
        ...
        self._last_eligible_codes = sorted(
            set(indicators_data["ts_code"].dropna().astype(str))
        )
        self.config["factor_input_count"] = len(self._last_eligible_codes)
        industry_data = self.data_repository.industry_classification(
            self._last_eligible_codes, as_of_date
        )
        ...
        result = compute_p_snapshot(
            ...
            industry_data,
            as_of_date,
            self._calculate_p_factors_from_mvp_indicators_pit,
        )
    ...
    def calculate_p_factors_batch_pit(
        ...
        return {
            ...
            "successful_dates": len(calc_dates)
            - (
                total_failed
                // max(len(stock_codes) if "stock_codes" in locals() else 1, 1)
            ),
    ...
    def detect_execution_mode(self, start_date: str, end_date: str) -> str:
        ...
        try:
            ...
            if result.empty or result.iloc[0]["count"] == 0:
                ...
                self.logger.info(
                    f"发现 {result.iloc[0]['count']} 条现有P因子数据，使用incremental模式"
                )
                return "incremental"
    ...
    def _get_trading_stock_codes(self, calc_date: str) -> List[str]:
        ...
        Returns:
            在交易股票代码列表
        """
        return self.data_repository.trading_stock_codes(calc_date)
    ...
    def _get_mvp_precomputed_indicators_pit(
        self, as_of_date: str, stock_codes: List[str]
    ) -> pd.DataFrame:
        ...
        """
        return self.data_repository.eligible_indicators(as_of_date, stock_codes)
    ...
    def _calculate_p_factors_from_mvp_indicators_pit(
        ...
        df = self._apply_industry_special_handling(
            df, as_of_date, industry_info=industry_info
        )
        ...
        if len(affected_stocks) > 0:
            self.logger.info(
                f"重新计算 {len(affected_stocks)} 只金融股的P评分 (GPA=NULL)"
            )
            ...
            for idx in df[mask_null_gpa].index:
                roe_score = (
                    df.loc[idx, "roe_excl_ttm_standardized"]
                    if "roe_excl_ttm_standardized" in df.columns
                    else 0
                )
                roa_score = (
                    df.loc[idx, "roa_excl_ttm_standardized"]
                    if "roa_excl_ttm_standardized" in df.columns
                    else 0
                )
        ...
        for col in output_columns:
            if col not in df.columns:
                if col == "ann_date":
                    ...
                    self.logger.warning("缺少ann_date字段，使用end_date作为临时值")
        ...
        self.logger.info(
            f"P因子计算完成: 平均评分 {df['p_score'].mean():.2f}, "
    ...
    def _apply_industry_special_handling(
        self,
        df: pd.DataFrame,
        as_of_date: str,
        industry_info: Optional[pd.DataFrame] = None,
        ...
        try:
            ...
            if industry_info is None:
                industry_info = self._get_industry_classification_pit(
            ...
            # 应用GPA特殊处理
            mask_special_gpa = df_with_industry["requires_special_gpa_handling"].eq(
                True
            )
            ...
            if len(special_stocks) > 0:
                self.logger.info(
                    f"对 {len(special_stocks)} 只金融股应用GPA特殊处理: {list(special_stocks)}"
                )

            # 移除临时列
            df_result = df_with_industry.drop(
                ["requires_special_gpa_handling", "gpa_calculation_method"], axis=1
            )
    ...
    def _get_industry_classification_pit(
        ...
        Returns:
            行业分类信息DataFrame
        """
        return self.data_repository.industry_classification(stock_codes, as_of_date)
    ...
    def _save_p_factors_mvp(self, p_factors: pd.DataFrame) -> tuple[int, Optional[str]]:
        ...
        count, checksum, _ = writer.write(
            p_factors,
            "p",
            calc_date,
            expected_codes=getattr(
                self, "_last_eligible_codes", p_factors["ts_code"].tolist()
            ),
            run_id=self.config.get("factor_run_id"),
            task_name=self.config.get("factor_task_name", "factor_p"),
            input_count=self.config.get("factor_input_count"),
            duration_ms=self.config.get("factor_duration_ms"),
            details={
                "formula_version": self.config.get("formula_version", "v2.0"),
    ...
    def _log_performance_stats(self, success_count: int, failed_count: int) -> None:
        ...
        if total_time > 0:
            self.logger.info(
                f"  查询时间: {stats['query_time']:.3f}s ({stats['query_time']/total_time*100:.1f}%)"
            )
            self.logger.info(
                f"  计算时间: {stats['calculation_time']:.3f}s ({stats['calculation_time']/total_time*100:.1f}%)"
            )
            self.logger.info(
                f"  保存时间: {stats['save_time']:.3f}s ({stats['save_time']/total_time*100:.1f}%)"
            )

    def check_mvp_data_availability(
        self, calc_date: str, stock_codes: List[str]
    ) -> Dict[str, Any]:
        """检查MVP预计算数据的可用性
        ...
        if result:
            ...
            coverage_rate = (
                (total_count / len(stock_codes) * 100) if len(stock_codes) > 0 else 0
            )
    ...
    def check_mvp_data_availability_pit(
        self, as_of_date: str, stock_codes: List[str]
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            if result and len(result) >= 7:
                ...
                coverage_rate = (
                    (unique_stocks / len(stock_codes) * 100)
...
ProductionPFactorCalculator = PFactorCalculator
