...
from alphahome.factors.core.computation import compute_g_snapshot
from alphahome.factors.core.context import ensure_factor_context
from alphahome.factors.core.data_repository import GFactorDataRepository
...
class GFactorCalculator:
    ...
    def __init__(
        ...
        self.data_repository = GFactorDataRepository(self.context, self.logger)
        ...
        self.es_params = {
            ...
            "yoy_match_tolerance_days": int(
                self.config.get("yoy_match_tolerance_days", 45)
            ),
            ...
            "enable_pooled_std_fallback": bool(
                self.config.get("enable_pooled_std_fallback", True)
            ),
        }
        self.logger.info(f"效率惊喜参数: {self.es_params}")
    ...
    def calculate_g_factors_pit(
        ...
        g_factor_results = self.compute_g_factors_pit(as_of_date, stock_codes)

        if g_factor_results.empty:
            self.logger.warning("G因子计算结果为空")
            return {
                "status": "no_data",
                "success_count": 0,
        ...
        self.config["factor_duration_ms"] = int((time.time() - start_time) * 1000)
        success_count, checksum = self._save_g_factor_results_pit(
            g_factor_results, as_of_date
        )
        ...
        return {
            "status": "success" if success_count else "no_data",
    ...
    def compute_g_factors_pit(
        self,
        as_of_date: str,
        stock_codes: List[str],
    ) -> pd.DataFrame:
        """Read and calculate one G snapshot without mutating factor tables."""
        ...
        if p_factor_data.empty:
            self._last_eligible_codes = []
            self.config["factor_input_count"] = 0
            return pd.DataFrame()
        same_day = (
            pd.to_datetime(p_factor_data["calc_date"]).dt.strftime("%Y-%m-%d")
            == as_of_date
        )
        self._last_eligible_codes = sorted(
            set(p_factor_data.loc[same_day, "ts_code"].dropna().astype(str))
        )
        if not self._last_eligible_codes:
            self.config["factor_input_count"] = 0
            return pd.DataFrame()
        self.config["factor_input_count"] = len(self._last_eligible_codes)
        calc_start = time.time()
        result = compute_g_snapshot(
            p_factor_data,
            as_of_date,
            self._calculate_g_factors_from_p_data_pit,
        )
        self.stats["calculation_time"] = time.time() - calc_start
        return result

    def _get_p_factor_historical_data_pit(
        self, as_of_date: str, stock_codes: List[str]
    ) -> pd.DataFrame:
        """基于PIT原则查询P因子历史数据
        ...
        Returns:
            P因子历史数据DataFrame
        """
        return self.data_repository.p_history(as_of_date, stock_codes)

    def _calculate_g_factors_from_p_data_pit(
        self, p_factor_data: pd.DataFrame, as_of_date: str
    ) -> pd.DataFrame:
        """基于P因子数据和PIT原则计算G因子
        ...
        for ts_code, group in p_factor_data.groupby("ts_code"):
            try:
                ...
                if latest_record is None:
                    continue
                if (
                    pd.Timestamp(latest_record["calc_date"]).date()
                    != pd.Timestamp(as_of_date).date()
                ):
                    raise ValueError(f"{ts_code} 缺少目标日P快照，禁止使用历史P回退")

                # 计算G因子子指标
                g_factors = self._calculate_g_subfactors(
                    group, latest_record, as_of_date
                )
    ...
    def _get_latest_p_factor_record(
        self, group: pd.DataFrame, as_of_date: str
    ) -> Optional[pd.Series]:
        """获取指定日期的最新P因子记录
    ...
    def _calculate_g_subfactors(
        self, group: pd.DataFrame, latest_record: pd.Series, as_of_date: str
        ...
        try:
            ...
            revenue_momentum = self._calculate_revenue_momentum(group, latest_record)

            # 4. 利润动量因子 (G_Profit_Momentum) - 基于财务指标
            profit_momentum = self._calculate_profit_momentum(group, latest_record)
    ...
    def _get_yoy_p_factor_data(
        self, group: pd.DataFrame, current_date: pd.Timestamp
    ) -> Optional[pd.Series]:
        """获取同比P因子数据 (1年前)
        ...
        yoy_candidates = group[
            (group["calc_date"] >= start_range) & (group["calc_date"] <= end_range)
        ]
    ...
    def _build_yoy_delta_series_52w(self, group: pd.DataFrame) -> List[float]:
        ...
        dates = pd.to_datetime(sorted_group["calc_date"], errors="coerce").to_numpy(
            dtype="datetime64[ns]"
        )
        scores = pd.to_numeric(sorted_group["p_score"], errors="coerce").to_numpy(
            dtype=float
        )
        interval = np.timedelta64(interval_weeks * 7, "D")
        ...
        for index, current_date in enumerate(dates):
            if np.isnat(current_date):
                continue
            try:
                target_date = current_date - interval
                left = int(np.searchsorted(dates, target_date - tolerance, side="left"))
                right = int(
                    np.searchsorted(dates, target_date + tolerance, side="right")
                )
                if left >= right:
                    continue
                candidate_dates = dates[left:right]
                differences = np.abs(
                    (candidate_dates - target_date)
                    .astype("timedelta64[ns]")
                    .astype(np.int64)
                )
                match_index = left + int(np.argmin(differences))
                if not np.isnan(scores[index]) and not np.isnan(scores[match_index]):
                    deltas.append(float(scores[index]) - float(scores[match_index]))
            except Exception:
                # 保持旧版单点异常隔离语义。
                continue
    ...
    def _calculate_efficiency_momentum(
        self, latest_record: pd.Series, yoy_data: Optional[pd.Series]
    ) -> float:
        """计算效率动量因子
    ...
    def _calculate_revenue_momentum(
        self, group: pd.DataFrame, latest_record: pd.Series
    ) -> float:
        """计算营收动量因子
    ...
    def _calculate_cross_sectional_rankings(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        # 效率惊喜排名：只对有效值进行排名
        df["rank_es"] = (
            df["g_efficiency_surprise"].rank(pct=True, na_option="keep") * 100
        )

        # 效率动量排名：只对有效值进行排名
        df["rank_em"] = (
            df["g_efficiency_momentum"].rank(pct=True, na_option="keep") * 100
        )
    ...
    def _save_g_factor_results_pit(
        self, g_factor_results: pd.DataFrame, calc_date: str
        ...
        try:
            ...
            writer = FactorSnapshotWriter(
                self.config.get("factor_writer_db_manager", self.context.db_manager)
            )
            success_count, checksum, _ = writer.write(
                g_factor_results,
                "g",
                calc_date,
                expected_codes=getattr(
                    self, "_last_eligible_codes", g_factor_results["ts_code"].tolist()
                ),
                run_id=self.config.get("factor_run_id"),
                task_name=self.config.get("factor_task_name", "factor_g"),
                input_count=self.config.get("factor_input_count"),
                duration_ms=self.config.get("factor_duration_ms"),
                details={
                    "formula_version": self.config.get("formula_version", "v1.1"),
                    "source_snapshot": self.config.get("factor_source_snapshot"),
                },
            )
            self.logger.info(
                f"成功保存 {success_count}/{len(g_factor_results)} 条G因子记录"
            )
            return success_count, checksum
    ...
    def calculate_g_factors_batch_pit(
        self, start_date: str, end_date: str, mode: Optional[str] = None
    ) -> Dict[str, Any]:
        ...
        # 2. 生成计算日期列表
        calc_dates = self.generate_calculation_dates(
            start_date, end_date, execution_mode
        )
        ...
        for i, calc_date in enumerate(calc_dates, 1):
            ...
            try:
                ...
                self.logger.info(
                    f"{calc_date} 计算完成: 成功 {result['success_count']}, 失败 {result['failed_count']}"
                )
    ...
    def detect_execution_mode(self, start_date: str, end_date: str) -> str:
        ...
        try:
            ...
            if result.empty or result.iloc[0]["count"] == 0:
                ...
                self.logger.info(
                    f"发现 {result.iloc[0]['count']} 条现有G因子数据，使用incremental模式"
                )
                return "incremental"
    ...
    def generate_calculation_dates(
        self, start_date: str, end_date: str, mode: str
    ) -> List[str]:
        """生成计算日期列表
    ...
    def _filter_missing_dates(self, dates: List[str]) -> List[str]:
        ...
        try:
            ...
            existing_series = pd.to_datetime(
                result["calc_date"], errors="coerce"
            ).dropna()
            ...
            self.logger.info(
                f"总日期: {len(dates)}, 已有数据: {len(existing_dates)}, 缺失数据: {len(missing_dates)}"
            )
    ...
    def _get_trading_stock_codes(self, calc_date: str) -> List[str]:
        ...
        Returns:
            在交易股票代码列表
        """
        return self.data_repository.same_day_p_codes(calc_date)
        ...
        for calc_date in calc_dates:
            try:
                ...
                if result.empty or result.iloc[0]["total_count"] == 0:
                    ...
                    row = result.iloc[0]
                    express_forecast_ratio = (
                        (row["express_count"] + row["forecast_count"])
                        / row["total_count"]
                        * 100
                    )

                    self.logger.info(
                        f"{calc_date}: 总记录 {row['total_count']}, "
