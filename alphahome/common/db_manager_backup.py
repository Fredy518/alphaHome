...
class DBManager:
    ...
    async def copy_from_dataframe(
        ...
        if conflict_columns and not isinstance(
            conflict_columns, list
        ):  # 确保 conflict_columns 是列表或 None
            raise ValueError("conflict_columns 必须是一个列表或None。")
        if conflict_columns is not None and not conflict_columns:  # 检查是否为空列表
            raise ValueError(
                "如果提供了 conflict_columns，则它不能为空列表。"
            )  # 原为 "conflict_columns cannot be an empty list if provided."
        ...
        if timestamp_column and timestamp_column not in df.columns:
            ...
            df[timestamp_column] = (
                None  # 使用None，假设数据库默认值或UPDATE时的NOW()会处理它
            )
        ...
        if (
            timestamp_column and timestamp_column not in df_columns
        ):  # 此条件理论上在上面的防御性代码后不应为真
            self.logger.warning(
    ...
    async def upsert(
        ...
        current_time = (
            datetime.now()
        )  # 如果需要在INSERT时填充，但主要依赖于UPDATE时的NOW()
        if (
            timestamp_column
        ):  # 此处的 timestamp_column 已被 UPSERT_FIX 保证为 'update_time' (如果原为None)
    ...
    async def create_table_from_schema(
        ...
        if not schema_def:  # schema 定义不能为空
            raise ValueError(
                f"无法创建表 '{table_name}'，未提供 schema_def (表结构定义)。"
            )
        ...
        async with self.pool.acquire() as conn:
            async with conn.transaction():  # 为DDL（数据定义语言）操作使用事务
                try:
                    ...
                    for col_name, col_def in schema_def.items():
                        if isinstance(col_def, dict) and "comment" in col_def:
                            ...
                            if comment_text is not None:
                                ...
                                self.logger.info(
                                    f"准备为列 '{table_name}.{col_name}' 添加注释: {comment_sql}"
                                )
                                await conn.execute(comment_sql)
                                self.logger.debug(
                                    f"为列 '{table_name}.{col_name}' 添加注释成功。"
                                )

                    # --- 2. 构建并执行 CREATE INDEX 语句 ---
                    # 为 date_column 创建索引 (如果需要且不是主键的一部分)
                    if date_column and date_column not in (
                        primary_keys or []
                    ):  # primary_keys可能为None
                        index_name_date = (
                            f"idx_{table_name}_{date_column}"  # 标准索引命名
                        )
                        create_index_sql_date = f'CREATE INDEX IF NOT EXISTS "{index_name_date}" ON "{table_name}" ("{date_column}");'
                        self.logger.info(
                            f"准备为 '{table_name}.{date_column}' 创建索引: {index_name_date}"
                        )
                    ...
                    if indexes and isinstance(indexes, list):
                        for index_def in indexes:
                            ...
                            if isinstance(index_def, dict):  # 索引定义是字典
                                index_columns_list = index_def.get(
                                    "columns"
                                )  # 获取列名或列名列表
                                ...
                                index_name = index_def.get(
                                    "name", f"idx_{table_name}_{safe_cols_for_name}"
                                )
                                unique = index_def.get(
                                    "unique", False
                                )  # 是否是唯一索引
                                ...
                                index_name = (
                                    f"idx_{table_name}_{index_def}"  # 标准索引命名
                                )
                            else:  # 未知格式
                                self.logger.warning(
                            ...
                            self.logger.info(
                                f"准备创建索引 '{index_name}' 于 '{table_name}({index_columns_str})': {unique_str.strip()}"
                            )
                    ...
                    self.logger.error(
                        f"创建表或索引 '{table_name}' 时失败: {e}", exc_info=True
                    )  # 记录详细异常信息
