...
class ProcessorTask(abc.ABC):
    ...
    async def post_process(self, data: pd.DataFrame, **kwargs) -> Dict[str, Any]:
        ...
        Returns:
            Dict[str, Any]: 后处理结果
        """
        if kwargs.get('save_result', True) and hasattr(self, 'result_table') and self.result_table:
    ...
    async def get_last_processed_date(self) -> Optional[datetime.date]:
        ...
        try:
            ...
            if result and result['last_date']:
                ...
                return result['last_date'].date() if hasattr(result['last_date'], 'date') else result['last_date']
