...
class BlockProcessor(abc.ABC):
            ...
            for block_id, block_params in blocks:
                ...
                if block_data_fetched is None:
                    ...
                    if main_data_df is None or main_data_df.empty:
                        self.logger.info(
                            f"块 {block_id} 的 'stock_data' 为空或不存在。"
                        )
                        is_block_empty = True
                else:
                    self.logger.warning(
                        f"块 {block_id} 的 _fetch_block_data 返回了未知类型的数据: {type(block_data_fetched)}"
                    )
                    is_block_empty = True

                if is_block_empty:
                    self.logger.warning(
                        f"块 {block_id} 数据无效或主要数据为空，跳过处理。"
                    )
                    continue
                ...
                if block_result is None or block_result.empty:
                    self.logger.warning(
                        f"块 {block_id} 的 process_block 返回了空结果，不保存。"
                    )
                elif (
                    ...
                    save_result = await self._save_block_result(
                        block_result, block_params, **kwargs
                    )
                ...
                # 日志
                self.logger.info(
                    f"块 {block_id} 处理完成，处理了 {len(block_result)} 行"
                )
    ...
    async def _split_into_date_blocks(
        self, **kwargs
        ...
        while current_start <= end_date:
            ...
            block_params = {"start_date": current_start, "end_date": current_end}
    ...
    async def _split_into_code_blocks(
        self, **kwargs
    ) -> List[Tuple[str, Dict[str, Any]]]:
        """按代码拆分成块
    ...
    async def _split_into_custom_blocks(
        self, **kwargs
    ) -> List[Tuple[str, Dict[str, Any]]]:
        """自定义块拆分
    ...
    async def _fetch_block_data(self, block_params: Dict[str, Any]) -> pd.DataFrame:
        ...
        # 添加日期条件
        if (
            "start_date" in block_params
            and "end_date" in block_params
    ...
    async def _save_block_result(
        self, data: pd.DataFrame, block_params: Dict[str, Any], **kwargs
    ) -> Dict[str, Any]:
        """保存块处理结果
