            ...
            if self.group_by:
                ...
                for col in existing_columns:
                    if self.method == 'mean':
                        ...
                        fill_value = result[col].mode().iloc[0] if not result[col].mode().empty else None
                        
                    if pd.notna(fill_value):
                        result[col] = result[col].fillna(fill_value)
                        
        elif self.method == 'ffill':
            result[existing_columns] = result[existing_columns].fillna(method='ffill', limit=self.limit)
            
        elif self.method == 'bfill':
            result[existing_columns] = result[existing_columns].fillna(method='bfill', limit=self.limit)
...
class DropNAOperation(Operation):
    ...
    async def apply(self, data: pd.DataFrame) -> pd.DataFrame:
        ...
        if self.axis == 0:
            ...
            self.logger.info(f"删除了 {rows_removed} 行 ({rows_removed/rows_before:.2%})")
        else:
            cols_removed = cols_before - cols_after
            self.logger.info(f"删除了 {cols_removed} 列 ({cols_removed/cols_before:.2%})")
