...
class MovingAverageOperation(Operation):
    ...
    async def apply(self, data: pd.DataFrame) -> pd.DataFrame:
        ...
        if self.group_by:
            # 检查分组列是否存在
            missing_columns = [col for col in self.group_by if col not in result.columns]
            if missing_columns:
            ...
            for name, group in grouped:
                ma_values = group[self.column].rolling(
                    window=self.window,
            ...
            # 不分组，直接计算
            result[self.result_column] = result[self.column].rolling(
                window=self.window,
                min_periods=self.min_periods,
                center=self.center
            ).mean()
        ...
        self.logger.info(f"计算了 {valid_count} 个 {self.result_column} 值，{null_count} 个空值")
