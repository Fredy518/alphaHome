...
@task_register()
class BarraExposuresDailyTask(BaseTask):
    """Barra 日截面因子暴露（MVP）。"""
    ...
    async def _fetch_data(self, stop_event=None, **kwargs) -> Optional[pd.DataFrame]:
        """Override BaseTask._fetch_data to implement data fetching."""
    ...
    def process_data(self, data: pd.DataFrame, stop_event=None, **kwargs) -> Optional[pd.DataFrame]:
        """Process data synchronously (called by BaseTask.execute)."""
        ...
        # Store for async post-processing
        self._processed_df = df
        return df

    async def _post_process_data(self, df: pd.DataFrame, **kwargs) -> pd.DataFrame:
        """Complete processing that requires async operations."""
    ...
    async def _save_data(self, data: pd.DataFrame, stop_event=None, **kwargs) -> Dict[str, Any]:
        ...
        # Complete async post-processing
        result = await self._post_process_data(data, **kwargs)
        ...
        return {"exposures_count": len(result)}
