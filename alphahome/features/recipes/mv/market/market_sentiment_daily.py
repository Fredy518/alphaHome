...
class MarketSentimentDailyMV(IncrementalTableView):
    ...
    # 窗口函数需要的额外回溯天数（新高新低需要 252 天历史）
    _lookback_days = 280

    create_sql = """
        CREATE TABLE features.mv_market_sentiment_daily AS
        WITH 
        -- ========== 市场宽度（MA 占比）==========
        ma_stats AS (
            SELECT 
        ...
        margin_stats AS (
            SELECT 
                ...
                -- 融资净买入（亿元）
                ((COALESCE(mg.total_margin_buy, 0) - COALESCE(mg.total_margin_repay, 0)) / 1e8)::NUMERIC AS margin_net_buy_billion,
                -- 融资/融券比
        ...
        SELECT 
            ...
            mg.margin_circ_ratio::NUMERIC,
            mg.margin_net_buy_billion::NUMERIC,
            mg.margin_short_ratio::NUMERIC,
            -- 血缘
            'tushare.stock_factor_pro,tushare.stock_limitlist,tushare.stock_st,tushare.stock_margin,tushare.stock_dailybasic'::TEXT AS _source_table,
    ...
    def get_incremental_sql(self, start_date: str, end_date: str) -> str:
        ...
        注意：新高新低窗口函数需要 252 天历史，这里使用 _lookback_days 回溯。
        """
        ...
        prices AS (
            ...
            FROM tushare.stock_factor_pro
            WHERE close_hfq IS NOT NULL
              AND trade_date >= '{lookback_date}' AND trade_date <= '{end_date}'
        ),
        rolling_extremes AS (
            SELECT
                ts_code,
                trade_date,
                close_hfq,
                COUNT(*) OVER (PARTITION BY ts_code ORDER BY trade_date ROWS BETWEEN 252 PRECEDING AND 1 PRECEDING) AS rolling_cnt_prev_252,
                MAX(close_hfq) OVER (PARTITION BY ts_code ORDER BY trade_date ROWS BETWEEN 252 PRECEDING AND 1 PRECEDING) AS rolling_max_prev_252,
                MIN(close_hfq) OVER (PARTITION BY ts_code ORDER BY trade_date ROWS BETWEEN 252 PRECEDING AND 1 PRECEDING) AS rolling_min_prev_252
            FROM prices
        ),
        new_high_low AS (
            SELECT
                trade_date,
                SUM(CASE WHEN rolling_cnt_prev_252 >= 126 AND close_hfq > rolling_max_prev_252 THEN 1 ELSE 0 END) AS new_high_count,
                SUM(CASE WHEN rolling_cnt_prev_252 >= 126 AND close_hfq < rolling_min_prev_252 THEN 1 ELSE 0 END) AS new_low_count
            FROM rolling_extremes
        ...
        limit_with_st AS (
            SELECT 
                l.trade_date, l.ts_code, l."limit", l.amount, l.open_times, l.limit_times, l.first_time,
                CASE WHEN s.ts_code IS NOT NULL THEN 1 ELSE 0 END AS is_st,
                CASE 
                    WHEN l.ts_code LIKE '%%.BJ' THEN 'bse'
                    WHEN l.ts_code LIKE '688%%.SH' OR l.ts_code LIKE '689%%.SH' OR l.ts_code LIKE '300%%.SZ' OR l.ts_code LIKE '301%%.SZ' THEN '20cm'
                    ELSE 'main'
                END AS board_type
            FROM tushare.stock_limitlist l
            LEFT JOIN tushare.stock_st s ON l.ts_code = s.ts_code AND l.trade_date = s.trade_date
            WHERE l.trade_date >= '{start_date}' AND l.trade_date <= '{end_date}'
        ),
        limit_stats AS (
            SELECT 
                trade_date,
                ...
                SUM(CASE WHEN "limit" = 'U' AND is_st = 0 AND limit_times >= 5 THEN 1 ELSE 0 END) AS consec_5_count,
                MAX(CASE WHEN "limit" = 'U' AND is_st = 0 THEN limit_times ELSE 0 END) AS max_consec,
                SUM(CASE WHEN "limit" = 'U' AND (CASE WHEN first_time ~ '^[0-9]{{2}}:[0-9]{{2}}:[0-9]{{2}}$' THEN REPLACE(SUBSTRING(first_time FROM 1 FOR 5), ':', '') WHEN first_time ~ '^[0-9]{{6}}$' THEN SUBSTRING(first_time FROM 1 FOR 4) ELSE NULL END)::INTEGER < 1000 THEN 1 ELSE 0 END) AS early_limit_count,
                SUM(CASE WHEN "limit" = 'U' AND is_st = 0 AND (CASE WHEN first_time ~ '^[0-9]{{2}}:[0-9]{{2}}:[0-9]{{2}}$' THEN REPLACE(SUBSTRING(first_time FROM 1 FOR 5), ':', '') WHEN first_time ~ '^[0-9]{{6}}$' THEN SUBSTRING(first_time FROM 1 FOR 4) ELSE NULL END)::INTEGER < 1000 THEN 1 ELSE 0 END) AS early_limit_count_non_st
            FROM limit_with_st
            GROUP BY trade_date
        ),
        ...
        -- ========== 融资融券 ==========
        margin_daily AS (
            SELECT trade_date, SUM(rzye) AS total_margin_balance, SUM(rzmre) AS total_margin_buy, SUM(rzche) AS total_margin_repay, SUM(rqye) AS total_short_balance
            FROM tushare.stock_margin
            WHERE trade_date >= '{start_date}' AND trade_date <= '{end_date}'
            GROUP BY trade_date
        ),
        market_cap AS (
            SELECT trade_date, SUM(circ_mv) AS total_circ_mv
            FROM tushare.stock_dailybasic
            WHERE trade_date >= '{start_date}' AND trade_date <= '{end_date}'
            GROUP BY trade_date
        ),
        margin_stats AS (
            SELECT 
                mg.trade_date, mg.total_margin_balance, mg.total_margin_buy, mg.total_margin_repay, mg.total_short_balance, mc.total_circ_mv,
                mg.total_margin_balance / 1e8 AS total_margin_balance_billion,
                mg.total_margin_buy / 1e8 AS total_margin_buy_billion,
                mg.total_margin_repay / 1e8 AS total_margin_repay_billion,
                mg.total_short_balance / 1e8 AS total_short_balance_billion,
        ...
        SELECT 
            m.trade_date,
            m.total_stock_count, m.above_ma60_count, m.above_ma90_count,
            m.above_ma60_count::FLOAT / NULLIF(m.total_stock_count, 0) AS above_ma60_ratio,
            m.above_ma90_count::FLOAT / NULLIF(m.total_stock_count, 0) AS above_ma90_ratio,
            m.up_count, m.down_count,
            m.up_count::FLOAT / NULLIF(m.down_count, 0) AS up_down_ratio,
            m.up_count::FLOAT / NULLIF(m.down_count + 1, 0) AS up_down_ratio_smooth,
            COALESCE(nh.new_high_count, 0) AS new_high_count,
            COALESCE(nh.new_low_count, 0) AS new_low_count,
            ...
            COALESCE(l.limit_up_bse, 0) AS limit_up_bse,
            COALESCE(l.limit_up_count, 0)::FLOAT / NULLIF(COALESCE(l.limit_down_count, 0), 0) AS limit_up_down_ratio,
            COALESCE(l.limit_up_count, 0)::FLOAT / NULLIF(COALESCE(l.limit_down_count, 0) + 1, 0) AS limit_up_down_ratio_smooth,
            COALESCE(l.limit_up_non_st, 0)::FLOAT / NULLIF(COALESCE(l.limit_down_non_st, 0), 0) AS limit_up_down_ratio_non_st,
            COALESCE(l.limit_up_non_st, 0)::FLOAT / (COALESCE(l.limit_down_non_st, 0) + 1) AS limit_up_down_ratio_non_st_smooth,
            COALESCE(l.limit_up_amount, 0) AS limit_up_amount,
            COALESCE(l.broken_limit_count, 0) AS broken_limit_count,
            ...
            COALESCE(l.broken_limit_count_non_st, 0) AS broken_limit_count_non_st,
            l.broken_limit_count_non_st::FLOAT / NULLIF(l.limit_up_non_st, 0) AS broken_limit_ratio_non_st,
            l.avg_open_times, l.avg_open_times_non_st,
            COALESCE(l.consec_2_count, 0) AS consec_2_count,
            COALESCE(l.consec_3_count, 0) AS consec_3_count,
            COALESCE(l.consec_5_count, 0) AS consec_5_count,
            COALESCE(l.max_consec, 0) AS max_consec,
