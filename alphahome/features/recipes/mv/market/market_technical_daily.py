...
优化说明:
- 支持增量刷新（默认刷新最近 30 天，窗口函数需回溯 60 天）
- 全量刷新耗时较长，建议使用增量模式
"""
...
class MarketTechnicalDailyMV(IncrementalTableView):
    ...
    # 窗口函数需要的额外回溯天数（60 日波动率需要最多 60 天历史）
    _lookback_days = 90

    create_sql = """
        CREATE TABLE features.mv_market_technical_daily AS
        WITH 
        -- ========== 基础数据（个股技术指标）==========
        base AS (
            SELECT 
        ...
        SELECT 
            ...
            -- 血缘
            'tushare.stock_factor_pro'::TEXT AS _source_table,
    ...
    def get_incremental_sql(self, start_date: str, end_date: str) -> str:
        ...
        注意：由于窗口函数需要历史数据，这里使用 _lookback_days 回溯更多数据，
        但只输出 start_date ~ end_date 范围内的结果。
        """
        # 回溯日期（用于窗口函数计算）
        ...
        base AS (
            ...
            FROM tushare.stock_factor_pro
            WHERE close_hfq IS NOT NULL AND close_hfq > 0
              AND trade_date >= '{lookback_date}'  -- 回溯用于窗口函数
        ...
        SELECT 
            ...
            CURRENT_DATE AS _data_version
        FROM base
        WHERE trade_date >= '{start_date}'  -- 只输出目标日期范围
          AND trade_date <= '{end_date}'
        GROUP BY trade_date
        """
