...
from alphahome.fetchers.tasks.fund.akshare_fund_fee_utils import (
    current_snapshot_date,
    normalize_fund_code,
)
from alphahome.fetchers.tasks.fund.akshare_fund_overview_em import (
    AkShareFundOverviewEmTask,
)
...
def test_normalize_fund_code_rejects_non_six_digit_symbols():
    assert normalize_fund_code("000001.OF") == "000001"
    assert normalize_fund_code("37001B.OF") is None
...
def test_fund_fee_purchase_schedule_parses_amount_and_discount_rates():
    ...
    processed = _run_pipeline(
        task, raw, fund_code="000001", indicator="申购费率（前端）"
    )
...
def test_fund_fee_redeem_schedule_parses_holding_days():
    task = AkShareFundFeeEmTask(db_connection=_MockDB(), update_type=UpdateTypes.FULL)
    raw = pd.DataFrame(
        {"适用期限": ["小于7天", "大于等于7天"], "赎回费率": ["1.50%", "0.50%"]}
    )

    processed = _run_pipeline(task, raw, fund_code="000001", indicator="赎回费率")

    assert processed["rule_type"].tolist() == [
        "holding_period_fee",
        "holding_period_fee",
    ]
...
def test_fund_code_tasks_use_fast_defaults():
    ...
    overview = AkShareFundOverviewEmTask(
        db_connection=_MockDB(), update_type=UpdateTypes.SMART
    )
    detail = AkShareFundIndividualDetailInfoXqTask(
        db_connection=_MockDB(), update_type=UpdateTypes.SMART
    )
...
@pytest.mark.asyncio
async def test_fund_fee_optional_operation_indicator_keyerror_is_no_data(monkeypatch):
    from alphahome.fetchers.tasks.fund import akshare_fund_fee_em as module
    ...
    data = await task.fetch_batch(
        {"fund_code": "000353", "symbol": "000353", "indicator": "运作费用"}
    )
...
@pytest.mark.asyncio
async def test_fund_fee_optional_purchase_indicator_falls_back_to_purchase_title(
    monkeypatch,
):
    from alphahome.fetchers.tasks.fund import akshare_fund_fee_em as module
...
async def test_fund_overview_smart_batches_skip_existing_month_codes_and_anchor_snapshot():
    ...
    assert batches == [
        {"fund_code": "000003", "symbol": "000003", "snapshot_date": str(anchor)}
    ]
...
def test_fund_overview_process_data_uses_snapshot_date_override():
    task = AkShareFundOverviewEmTask(
        db_connection=_MockDB(), update_type=UpdateTypes.SMART
    )
    raw = pd.DataFrame(
        [
...
@pytest.mark.asyncio
async def test_fund_overview_skips_non_six_digit_symbol_without_api_call():
    task = AkShareFundOverviewEmTask(
        db_connection=_MockDB(), update_type=UpdateTypes.SMART
    )
    ...
    class _FailIfCalledApi:
        async def call(self, **kwargs):
            raise AssertionError("unsupported symbol must not reach AkShare")

    task.api = _FailIfCalledApi()

    data = await task.fetch_batch(
        {"fund_code": "37001B", "symbol": "37001B", "snapshot_date": "2026-09-03"}
    )
...
async def test_fund_detail_xq_smart_batches_skip_existing_month_codes_and_anchor_snapshot():
    ...
    task = AkShareFundIndividualDetailInfoXqTask(
        db_connection=db, update_type=UpdateTypes.SMART
    )
    ...
    assert hasattr(
        db.month_args_by_table["fund_individual_detail_info_xq"][0], "toordinal"
    )
...
@pytest.mark.asyncio
async def test_fund_detail_xq_missing_payload_error_is_no_data(
    ...
    monkeypatch.setattr(
        task,
        "_read_available_rate_tables",
        lambda symbol, timeout: None,
    )

    data = await task.fetch_batch(
        {"fund_code": "000014", "symbol": "000014", "timeout": 10.0}
    )
...
@pytest.mark.asyncio
async def test_fund_detail_xq_missing_withdraw_table_preserves_available_sections(
    monkeypatch,
):
    task = AkShareFundIndividualDetailInfoXqTask(
        db_connection=_MockDB(), update_type=UpdateTypes.SMART
    ...
    class _FakeApi:
        async def call(self, **kwargs):
            ...
            raise AkShareAPIError(
                "akshare.fund_individual_detail_info_xq 调用失败: 'withdraw_rate_table'"
            )

    fallback = pd.DataFrame(
        [
            {"费用类型": "买入规则", "条件或名称": "0万<买入金额<100万", "费用": 0.8},
    ...
    monkeypatch.setattr(
        task,
        "_read_available_rate_tables",
        lambda symbol, timeout: fallback,
    )

    data = await task.fetch_batch(
        {"fund_code": "009353", "symbol": "009353", "timeout": 10.0}
    )

    assert data["fee_type"].tolist() == ["买入规则", "其他费用"]
    assert data["fee_value"].tolist() == [0.8, 1.2]
