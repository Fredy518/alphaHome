...
def test_only_current_eligible_input_gaps_block_p(isolated_database_url, missing_eligible):
    ...
    db.execute_sync("""
        ...
        INSERT INTO pit.pit_income_quarterly
          ...
          FROM unnest(ARRAY['F','D','M','U','B','N']) code;
        INSERT INTO pit.pit_balance_quarterly
          SELECT ts_code, end_date, ann_date, data_source, 100 FROM pit.pit_income_quarterly WHERE ts_code<>'B';
        INSERT INTO tushare.stock_basic
          ...
          FROM unnest(ARRAY['F','D','U','B','N']) code;
    ...
    try:
        ...
        assert report["delisted"] == report["missing_master"] == report["unconverted"] == report["missing_balance"] == report["not_yet_listed"] == 1
        ...
        assert blockers == (["pit_input_eligibility:missing=1"] if missing_eligible else [])
    finally:
