...
# 配置日志
log_filename = f'update_cashflow_task_{datetime.now().strftime("%Y%m%d")}.log'
logging.basicConfig(
    level=logging.INFO,
...
logger = logging.getLogger('update_cashflow_task')

# 定义目标任务
TARGET_TASK_NAME = "tushare_stock_cashflow" # 现金流量表

def calculate_report_dates(args: argparse.Namespace) -> Tuple[str, str]:
    """计算基于季度或年度选项的报告期范围 (与利润表脚本相同)
    Args:
        args: 命令行参数
    Returns:
...
async def update_task(args: argparse.Namespace) -> Dict[str, Any]:
    ...
    try:
        ...
        logger.info(f"开始更新现金流量表任务: {task_name}")
        ...
        if result is None:
            logger.warning(f"任务 {task_name} 返回的结果为None")
            return {'task_name': task_name, 'status': 'error', 'error': '任务执行返回None结果', 'rows': 0, 'failed_batches': 0}
        if not isinstance(result, dict):
            logger.warning(f"任务 {task_name} 返回的结果不是字典类型: {type(result)}")
            return {'task_name': task_name, 'status': 'error', 'error': f'任务执行返回非字典结果: {type(result)}', 'rows': 0, 'failed_batches': 0}
...
async def main():
    parser = argparse.ArgumentParser(description='现金流量表数据更新工具')
    ...
    try:
        ...
        logger.error(f"现金流量表数据更新过程中发生错误: {str(e)}", exc_info=True)
        if result is None:
    ...
    logger.info(f"现金流量表数据更新执行完毕。总耗时: {duration}")

if __name__ == "__main__":
    """
    使用示例:
    1. 默认增量更新（智能模式，回溯1年）:
       python examples/update_cashflow_task.py
    2. 更新最近N个季度:
       python examples/update_cashflow_task.py --quarters 4
    # ... (其他示例与income类似)
    6. 全量更新:
       python examples/update_cashflow_task.py --full-update
    """
    asyncio.run(main()) 