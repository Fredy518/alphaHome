...
# 配置日志
log_filename = f'update_fina_indicator_task_{datetime.now().strftime("%Y%m%d")}.log'
logging.basicConfig(
    level=logging.INFO,
...
logger = logging.getLogger('update_fina_indicator_task')

# 定义目标任务
TARGET_TASK_NAME = "tushare_stock_fina_indicator" # 财务指标

def calculate_report_dates(args: argparse.Namespace) -> Tuple[str, str]:
    """计算基于季度或年度选项的报告期范围 (逻辑与 income/cashflow 脚本相同)
    Args:
        args: 命令行参数
    Returns:
...
async def update_task(args: argparse.Namespace) -> Dict[str, Any]:
    """更新财务指标任务的数据 (逻辑与 income/cashflow 脚本类似)
    Args:
        args: 命令行参数
    Returns:
        Dict: 更新结果
    ...
    try:
        ...
        logger.info(f"开始更新财务指标任务: {task_name}")
        ...
        if args.full_update:
            ...
            result = await task.execute(start_date=args.start_date, end_date=end_date, **update_kwargs)
        else:
            logger.info(f"任务 {task_name}: 执行智能增量更新，回溯1年")
            result = await task.smart_incremental_update(lookback_days=365, **update_kwargs)

        # Result handling (same as previous scripts, NaN handled in DBManager)
        ...
        # Standardize rows/failed_batches to int, default 0
        rows = result.get('rows')
        failed_batches = result.get('failed_batches')
        standardized_result = {
            'task_name': task_name,
            'status': result.get('status', 'unknown'),
            'rows': 0 if rows is None or (isinstance(rows, float) and np.isnan(rows)) else int(rows),
            'failed_batches': 0 if failed_batches is None or (isinstance(failed_batches, float) and np.isnan(failed_batches)) else int(failed_batches)
        }
...
async def main():
    parser = argparse.ArgumentParser(description='财务指标数据更新工具')
    # Arguments remain the same as income/cashflow script
    ...
    logger.info(f"开始执行财务指标 ({TARGET_TASK_NAME}) 数据更新...")
    ...
    try:
        ...
        logger.error(f"财务指标数据更新过程中发生错误: {str(e)}", exc_info=True)
        if result is None:
    ...
    logger.info(f"财务指标数据更新执行完毕。总耗时: {duration}")

if __name__ == "__main__":
    """
    使用示例:
    1. 默认增量更新（智能模式，回溯1年）:
       python examples/update_fina_indicator_task.py
    2. 更新最近N个季度:
       python examples/update_fina_indicator_task.py --quarters 4
    3. 更新最近N年:
       python examples/update_fina_indicator_task.py --years 2
    4. 更新特定报告期:
       python examples/update_fina_indicator_task.py --report-period 202103
    5. 指定日期范围更新:
       python examples/update_fina_indicator_task.py --start-date 20230101 --end-date 20230630
    6. 全量更新:
       python examples/update_fina_indicator_task.py --full-update
    """
    asyncio.run(main())