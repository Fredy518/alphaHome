...
# 配置日志
log_filename = f'update_forecast_task_{datetime.now().strftime("%Y%m%d")}.log'
logging.basicConfig(
    level=logging.INFO,
...
logger = logging.getLogger('update_forecast_task')

# 定义目标任务
TARGET_TASK_NAME = "tushare_stock_forecast" # 业绩预告

def calculate_report_dates(args: argparse.Namespace) -> Tuple[str, str]:
    """计算基于季度或年度选项的报告期范围 (与之前脚本相同)
    Args:
        args: 命令行参数
    Returns:
        Tuple[str, str]: 开始日期和结束日期，格式为YYYYMMDD
    """
    # Logic is identical to the previous scripts
    today = datetime.now()
    ...
    if args.report_period:
        ...
        if today > current_quarter_end: latest_quarter_year, latest_quarter_month = current_year, current_quarter_month
        else:
...
async def update_task(args: argparse.Namespace) -> Dict[str, Any]:
    ...
    try:
        ...
        logger.info(f"开始更新业绩预告任务: {task_name}")
        update_kwargs = {'show_progress': args.show_progress}
        result = None

        # Determine start/end dates based on args (same logic as previous scripts)
        if args.full_update:
            # Forecast task might primarily use 'ann_date' or 'end_date' filtering in Tushare
            # We pass start/end_date for consistency, task should handle relevant filtering.
            start_date = task.default_start_date if hasattr(task, 'default_start_date') else '19900101'
            end_date = datetime.now().strftime('%Y%m%d')
            logger.info(f"任务 {task_name}: 执行全量更新，从 {start_date} 到 {end_date} (基于公告日期)")
            result = await task.execute(start_date=start_date, end_date=end_date, **update_kwargs) # Pass dates, task uses relevant ones
        elif args.report_period or args.quarters or args.years:
            start_date, end_date = calculate_report_dates(args)
            logger.info(f"任务 {task_name}: 执行财务报告期更新，从 {start_date} 到 {end_date} (基于公告日期)")
            # Forecast uses ann_date, not report_period directly. We use end_date derived from calculation.
            result = await task.execute(start_date=start_date, end_date=end_date, **update_kwargs)
        elif args.start_date:
            start_date = args.start_date
            end_date = args.end_date or datetime.now().strftime('%Y%m%d')
            logger.info(f"任务 {task_name}: 执行日期范围更新，从 {start_date} 到 {end_date} (基于公告日期)")
            result = await task.execute(start_date=start_date, end_date=end_date, **update_kwargs)
        else:
            # Smart incremental for forecast might look back less, e.g., 90 days for recent announcements?
            # Keeping 1 year for consistency, task's implementation dictates actual behavior.
            logger.info(f"任务 {task_name}: 执行智能增量更新，回溯1年 (基于公告日期)")
...
def summarize_result(result: Dict[str, Any], args: argparse.Namespace) -> None:
    ...
    Args:
        ...
        args: 命令行参数
    """
    logger.info("业绩预告数据更新结果汇总:")
    if not isinstance(result, dict): logger.error(f"结果格式错误: {result}"); return

    # Display update parameters (same as previous scripts, note context about announcement date)
    if args.report_period: logger.info(f"查询公告日期对应的报告期: {format_report_period(args.report_period)}") # Clarify it's about ann_date
    elif args.quarters: logger.info(f"查询最近 {args.quarters} 个季度的公告")
    elif args.years: logger.info(f"查询最近 {args.years} 年的公告")
    elif args.start_date: logger.info(f"查询公告日期范围: {args.start_date} 至 {args.end_date or datetime.now().strftime('%Y%m%d')}")
    elif args.full_update: logger.info("更新模式: 全量更新 (基于公告日期)")
    else: logger.info("更新模式: 智能增量更新 (默认回溯1年，基于公告日期)")
...
async def main():
    ...
    # Arguments are the same as previous scripts
    parser.add_argument('--quarters', type=int, help='查询最近 N 个季度的公告数据')
    parser.add_argument('--years', type=int, help='查询最近 N 年的公告数据')
    parser.add_argument('--report-period', help='查询指定报告期对应的公告 (格式: YYYYMM)')
    parser.add_argument('--start-date', help='查询公告起始日期 (格式: YYYYMMDD)')
    parser.add_argument('--end-date', help='查询公告结束日期 (格式: YYYYMMDD)')
    parser.add_argument('--full-update', action='store_true', help='全量更新模式 (基于公告日期)')
    parser.add_argument('--auto', action='store_true', help='自动增量模式 (基于公告日期)')
    parser.add_argument('--show-progress', action=argparse.BooleanOptionalAction, default=True, help='是否显示进度条')
    args = parser.parse_args()

    start_time = datetime.now()
    logger.info(f"开始执行业绩预告 ({TARGET_TASK_NAME}) 数据更新...")
    ...
    try:
        ...
        logger.error(f"业绩预告数据更新过程中发生错误: {str(e)}", exc_info=True)
        if result is None:
    ...
    logger.info(f"业绩预告数据更新执行完毕。总耗时: {duration}")

if __name__ == "__main__":
    """
    使用示例:
    1. 默认增量更新（智能模式，基于公告日期回溯1年）:
       python examples/update_forecast_task.py
    # ... (其他示例与之前类似, 注意日期参数是基于公告日期)
    6. 全量更新:
       python examples/update_forecast_task.py --full-update
    """
    asyncio.run(main()) 